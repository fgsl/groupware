...
class Config extends AbstractConfig
{
     ...
     * @var string
     */
    const USERBACKEND = 'User_BackendConfiguration';
     ...
     * @var string
     */
    const USERBACKENDTYPE = 'User_BackendType';
    ...
    */
    ...
    protected static $_properties = array(
        ...
        self::USERBACKEND => array(
            ...
            'setBySetupModule'      => TRUE,
            'content'               => array(
                User::DEFAULT_USER_GROUP_NAME_KEY => array(
                    'type'                      => Config::TYPE_STRING,
                ),
                User::DEFAULT_ADMIN_GROUP_NAME_KEY => array(
                    'type'                      => Config::TYPE_STRING,
                ),
                'host'                      => array(
                    'type'                      => Config::TYPE_STRING,
        ...
        // Retrieve via Core::getInstallLogo(), never use directly!
    ...
    public function getClientRegistryConfig()
    {
        ...
        $clientProperties = new Struct(array());
        $userApplications = Core::getUser()->getApplications(TRUE);
        foreach ($userApplications as $application) {
            $config = AbstractConfig::factory($application->name);
            if ($config) {
                $clientProperties[$application->name] = new Struct(array());
                ...
                foreach ((array) $properties as $name => $definition) {
                    ...
                    if (isset($definition['clientRegistryInclude']) && $definition['clientRegistryInclude'] === TRUE)
                    {
                        ...
                        try {
                            ...
                            if ($type) {
                                $val = $config->{$name};
                                if ($val && Config::TYPE_RECORD === $type &&
                                        isset($definition[Config::TYPE_RECORD_CONTROLLER])) {
                                    try {
                                        $val = $definition[Config::TYPE_RECORD_CONTROLLER]::getInstance()
                                            ->get($val);
                                    } catch(\Exception $e) {
                                        Exception::log($e);
                                    }
                                }
                                $configRegistryItem = new Struct(array(
                                    'value' => $val,
                                    'definition' => new Struct($definition),
                                ), null, null, array(
                                    'value' => array('type' => $definition['type']),
                                    'definition' => array('type' => AbstractConfig::TYPE_ARRAY, 'class' => 'Fgsl\Groupware\Groupbase\Config\Struct')
                                ));
                                if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__
     ...
     * @return AbstractConfig  $configClass
     */
