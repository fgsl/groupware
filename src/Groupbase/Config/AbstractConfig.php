...
abstract class AbstractConfig implements ConfigInterface
{
    ...
    public function get($name, $default = NULL)
    {
        ...
        $dbAvailable = 'logger' === $name ? false :
            (('database' === $name || 'caching' === $name) ? Core::hasDb() : true);
    ...
    public function set($_name, $_value)
    {
        ...
        $configRecord = new Tinebase_Model_Config(array(
            "application_id"    => Application::getInstance()->getApplicationByName($this->_appName)->getId(),
            "name"              => $_name,
    ...
    protected function _getConfigFileData()
    {
        if (! self::$_configFileData) {
            ...
            if (isset(self::$_configFileData['confdfolder'])) {
                $tmpDir = Core::guessTempDir(self::$_configFileData);
    ...
    protected function _getAppDefaultsConfigFileData()
    {
        ...
        if (file_exists($configFilename)) {
            ...
            if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::'
    ...
    */
    protected function _loadAllAppConfigsInCache()
    {
        if (empty($this->_appName)) {
            if (Core::isLogLevel(LogLevel::WARN)) Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' appName not set');
            $this->_cachedApplicationConfig = array();
        }

        if (! Application::getInstance()->getInstance()->isInstalled('Tinebase')) {
            $this->_cachedApplicationConfig = array();
            return;
        }
        ...
        try {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Loading all configs for app ' . $this->_appName);
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Found ' . count($allConfigs) . ' configs.');
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($allConfigs->toArray(), TRUE));
    ...
    public function clearCache($appFilter = null, $keepMemoryCache = false)
    {
        ...
        if (Core::get(Core::SHAREDCACHE)) {
            if (isset($appFilter)) {
                ...
                $appName = $key === 'name' ? $value : Application::getInstance()->getApplicationById($value)->name;
            } else {
                $appName = $this->_appName;
            }
            Core::getCache()->remove('cachedAppConfig_' . $appName);
        }

        Tinebase_Cache_PerRequest::getInstance()->reset('Tinebase_Config_Abstract');

        $cachedConfigFile = Core::guessTempDir() . DIRECTORY_SEPARATOR . 'cachedConfig.inc.php';
        if (file_exists($cachedConfigFile)) {
            unlink($cachedConfigFile);
        }

        // reset class caches last because they would be filled again by Core::guessTempDir()
        ...
        // after clearCache a call to Core::getConfig()->logger would cause a __get('logger') recursion
