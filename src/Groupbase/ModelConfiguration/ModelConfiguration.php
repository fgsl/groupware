...
class ModelConfiguration extends ConfigConst {
     ...
     * * records                 1:n - Relation      -        Array of Record.data Objects  RecordSet -                                 -
    ...
    public function __construct($modelClassConfiguration)
    {
        ...
        foreach ($this->_fields as $fieldKey => &$fieldDef) {
            ...
            if ($fieldDef[self::TYPE] === 'keyfield') {
                $fieldDef['length'] = 40;
                if (Application::getInstance()->isInstalled($this->_applicationName)) {
...
//                 }
    ...
    public function getTableName()
    {
        if (is_array($this->_table) && isset($this->_table['name'])) {
            ...
            try {
                ...
                if (null !== ($appInstance = Core::getApplicationInstance(
    ...
    {
        return Core::getApplicationInstance($this->_appName, $this->_modelName);
    }
    ...
    protected function _isAvailable($_fieldConfig)
    {
        ...
        if (isset($fieldConfig['appName'])) {
            if (!isset(self::$_availableApplications[$fieldConfig['appName']])) {
                self::$_availableApplications[$fieldConfig['appName']] = Application::getInstance()
        ...
        if ($result && isset($fieldConfig['feature'])) {
            ...
            if (! $result && Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__
     ...
     * @todo support '*'
     */
    public function resolve(RecordSet $_records, $_what)
    {
        if (count($_records) == 0) {
            return;
        }
     ...
     * @param array $_field
     */
    protected function _resolveVirtualRelations(RecordSet $_records, $_field)
    {
     ...
     */
    protected function _resolveRelations(RecordSet $_records)
    {
     ...
     */
    public static function resolveRecordsPropertiesForRecordSet(RecordSet $_records, $modelConfiguration, $isSearch = false)
    {
        ...
        foreach ($resolveFields as $fieldKey => $c) {
            ...
            foreach ($_records as $record) {
                if (null === $record->{$fieldKey}) {
                    $record->{$fieldKey} = new RecordSet($foreignRecordClass);
                }
            }
        }
    }
