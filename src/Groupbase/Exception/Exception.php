...
class Exception extends \Exception
{
    ...
    {
        $logMethod = $exception instanceof Exception ? $exception->getLogLevelMethod() : 'err';
        $logLevel = strtoupper($logMethod);

        Core::getLogger()->$logMethod(__METHOD__ . '::' . __LINE__ . ' ' . get_class($exception)
            . ' -> ' . $exception->getMessage());

        if ($additionalData) {
            Core::getLogger()->$logMethod(__METHOD__ . '::' . __LINE__ . ' Data: ' . print_r($additionalData, true));
        }
        ...
        if (Core::isLogLevel(constant("LogLevel::$logLevel")) && ! $suppressTrace) {
            $traceString = $exception->getTraceAsString();
            $traceString = self::_replaceBasePath($traceString);
            $traceString = self::_removeCredentials($traceString);
            Core::getLogger()->$logMethod(__METHOD__ . '::' . __LINE__ . ' ' . $traceString);
        }
    }
    ...
    {
        $sentryClient = Core::getSentry();
        if (! $sentryClient) {
            return;
        }

        if ($exception instanceof Exception && ! $exception->logToSentry()) {
            return;
        }

        Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Sending exception to Sentry');
        $sentryClient->captureException($exception, array(
            'extra' => array(
                'tinebaseId' => Core::getTinebaseId(),
            ),
        ));
    }
