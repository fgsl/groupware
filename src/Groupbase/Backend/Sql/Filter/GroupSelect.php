 ...
 * $select2 = new GroupSelect($select1);
 * $select2->where(COND2.1)
 * $select2->appendWhere(Select::SQL_OR);
 *
...
class GroupSelect
{
     ...
     *
     * @param  Select|GroupSelect $_select
     */
    public function __construct($_select)
    {
        $this->_select = $_select;
        $this->_adapter = Core::getDb();
    }
    ...
    {
        $this->_parts[Select::WHERE][] = $this->_where($cond, $value, $type, true);
    ...
    {
        $this->_parts[Select::WHERE][] = $this->_where($cond, $value, $type, false);
    ...
    protected function _where($condition, $value = null, $type = null, $bool = true)
    {
        ...
        $cond = "";
        if (! empty($this->_parts[Select::WHERE]) && end($this->_parts[Select::WHERE]) !== '(') {
            if ($bool === true) {
                $cond = Select::SQL_AND . ' ';
            } else {
                $cond = Select::SQL_OR . ' ';
     ...
     */
    public function appendWhere($_concatenationCondition = Select::SQL_AND)
    {
        if (! empty($this->_parts[Select::WHERE])) {
            $method = $_concatenationCondition == Select::SQL_OR ? 'orWhere' : 'where';
    ...
    {
        return implode(' ', $this->_parts[Select::WHERE]);
    }
}
