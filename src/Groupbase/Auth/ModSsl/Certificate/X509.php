...
use Fgsl\Groupware\Groupbase\Auth\ModSsl\Model\DigitalCertificateValidation;
...
//        $dateTimezone = new DateTimeZone(Core::getUserTimezone());
//        $locale = new Zend_Locale($_translation->getAdapter()->getLocale());
        
        // Date valid from
        
        $this->validFrom = Translation::dateToStringInTzAndLocaleFormat(new DateTime($c['validFrom_time_t']));
        
        // Date valid to
        $this->validTo = Translation::dateToStringInTzAndLocaleFormat(new DateTime($c['validTo_time_t']));
    ...
    static public function readCertificatesFile($filename){
        ...
        if ($fileContents !== false) {
            ...
            Core::getLogger()->crit(__METHOD__ . '::' . __LINE__
        ...
        foreach ($match[0] as $pemCertificate) {
            //$x509Certificate = CertificateFactory::buildCertificate($pemCertificate, true);
            $certificateModel = DigitalCertificateValidation::createFromCertificate($pemCertificate, TRUE, TRUE);
    ...
    {
        $cdError = Translation::getTranslation('Tinebase')->_('Couldn\'t verify if certificate was revoked.');
        ...
        $certTempFile = self::generateTempFilename($temporary_files, Core::getTempDir());
        ...
        if(isset($aux[1])) {
            if($aux[1] != 'OK') {
                foreach($out as $item) {
                    $aux = explode(':',$item);
                    if(isset($aux[1])) {
                        $this->status['errors'][] = Translation::getTranslation('Tinebase')->_(trim($aux[1]));
                        $this->status['isValid'] = false;
                    }
                }
                return;
    ...
    {
        $error = Translation::getTranslation('Tinebase')->_('Couldn\'t verify if certificate was revoked.');
