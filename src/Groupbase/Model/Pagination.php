 ...
 */
class Pagination extends AbstractRecord
{
    /**
    ...
    protected function _readModelConfig()
    {
        ...
        $this->_customFields = CustomField::getInstance()
        ...
        foreach ($this->xprops('sort') as &$field) {
            if (isset($mapping[$field])) {
                ...
                if (null === ($relatedMC = $relatedModel::getConfiguration())) {
                    $e = new InvalidArgument('related model not a modelconfig model in pagination: '
                        . $relatedModel);
                    Exception::log($e);
                    continue;
                }
    ...
    public function appendSort($_select)
    {
        if (!empty($this->sort) && !empty($this->dir)){
            $tableName = null;
            foreach ($_select->getPart(Select::FROM) as $key => $from) {
                if (Select::FROM === $from['joinType']) {
                    if (is_numeric($key)) {
            ...
            $_select->order($this->_getSortCols($tableName, $_select->getPart(Select::COLUMNS)));
        }        
    }
