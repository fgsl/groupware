 ...
 */
class Container extends AbstractRecord
{
    /**
    ...
    protected static $_modelConfiguration = array(
        ...
        'associations' => [
            \Doctrine\ORM\Mapping\ClassMetadataInfo::MANY_TO_ONE => [
                'container_id_fk' => [
                    'targetEntity' => ModelApplication::class,
        ...
        'fields' => array(
            ...
            'hierarchy'         => array(
                ...
                self::INPUT_FILTERS     => [NotEmpty::class => false],
    ...
    /**
     * converts a int, string or Container to a containerid
     *
     * @param   int|string|Container $_containerId the containerid to convert
     * @return  string
    ...
    static public function convertContainerId($_containerId)
    {
        ...
        if ($_containerId instanceof Container) {
        ...
        if (false === ctype_alnum($id)) {
            throw new InvalidArgument('No container id set.');
        }
    ...
    public function setFromArray(array &$_data)
    {
        ...
        switch ($this->type) {
            case Container::TYPE_SHARED:
                $this->path = "/{$this->type}/{$this->getId()}";
                break;
                
            case Container::TYPE_PERSONAL:
    ...
    public function getOwner()
    {
        ...
        if (! $this->owner_id) {
            ...
            $allGrants = GroupbaseContainer::getInstance()->getGrantsOfContainer($this, true);
            
            // pick the first user with admin grants
            foreach ($allGrants as $grants) {
                if ($grants->{Grants::GRANT_ADMIN} === true) {
                    $this->owner_id = $grants->account_id;
                    break;
                }
            }
            if (! $this->owner_id) {
                throw new NotFound('Could not find container admin');
            }
        }
    ...
    {
        return $this->type == Container::TYPE_PERSONAL 
            && $this->getOwner() == User::convertUserIdToInt($account);
    }
    ...
    public static function resolveContainerOfRecord($_record, $_containerProperty = 'container_id')
    {
        ...
        try {
            $container = GroupbaseContainer::getInstance()->getContainerById($_record->{$_containerProperty});
        } catch (NotFound $tenf) {
            if (Core::isLogLevel(LogLevel::NOTICE)) Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' ' . $tenf);
            return;
        }
    ...
    {
        $this->account_grants = GroupbaseContainer::getInstance()->getGrantsOfAccount(Core::getUser(), $this);
    ...
    public static function pathIsPersonalNode($_path)
    {
        ...
        if (preg_match("/^\/personal\/([0-9a-z_\-]+)$/i", $_path, $matches)) {
            ...
            return $matches[1] == User::CURRENTACCOUNT ? Core::getUser()->getId() : $matches[1];
        }
