...
use Fgsl\Groupware\Groupbase\Model\Filter\Relation as FilterRelation;
 ...
 * @subpackage  Filter
 */
class FilterGroup implements \Iterator
{
    ...
    /**
     * @var FilterGroup|null reference to parent group
     */
    ...
    protected function _createForeignRecordFilterFromArray($_filterData)
    {
        ...
        switch ($_filterData['value']['linkType']) {
            case 'relation':
                ...
                $filter = new FilterRelation($filterData);
                break;
            ...
            case 'foreignId':
                ...
                break;
            default:
                Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Skipping filter (foreign record filter syntax problem) -> ' 
    ...
    protected function _getModelNameFromLinkInfo($_linkInfo, $_modelKey)
    {
        if (   ! in_array($_modelKey, array('modelName', 'filterName')) 
            ...
            throw new InvalidArgument('Foreign record filter needs appName and modelName or filterName');
        }

        $appName = str_replace('_', '', $_linkInfo['appName']);
        
        if (! Application::getInstance()->isInstalled($appName) || (is_object(Core::getUser()) && ! Core::getUser()->hasRight($appName, AbstractRights::RUN))) {
            throw new AccessDenied('No right to access application');
        }
        
        $modelName = $appName . '_Model_' . str_replace('_', '', $_linkInfo[$_modelKey]);
        
        if (! class_exists($modelName)) {
            throw new InvalidArgument('Model does not exist');
        }
    ...
    protected function _createStandardFilterFromArray($_filterData)
    {
        ...
        if (empty($fieldModel)) {
            ...
            Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Skipping filter (filter syntax problem) -> ' 
                . static::class . ' with filter data: ' . print_r($_filterData, TRUE));
        }
    }
    ...
    protected function _addCustomFieldFilter($_filterData)
    {
        ...
        $customFieldConfig = CustomField::getInstance()->getCustomFieldByNameAndApplication(
     ...
     *
     * @param  AbstractFilter $_filter
     * @param  boolean $_setFromArray
     * @return FilterGroup this
     * @throws InvalidArgument
     */
    public function addFilter(AbstractFilter $_filter, $_setFromArray = FALSE)
    {
        if (! $_filter instanceof AbstractFilter) {
            if ($_filter instanceof FilterGroup) {
                return $this->addFilterGroup($_filter);
            }
            throw new InvalidArgument('Filters must be of instance AbstractFilter');
        }
        
        if (! $_setFromArray && $_filter instanceof AclFilter) {
    ...
    public function addFilterGroup($_filtergroup)
    {
        ...
        if (! $_filtergroup instanceof FilterGroup) {
            if ($_filtergroup instanceof AbstractFilter) {
                return $this->addFilter($_filtergroup);
            }
            throw new InvalidArgument('Filters must be of instance FilterGroup');
        }
    ...
    public function createFilter($_fieldOrData, $_operator = NULL, $_value = NULL)
    {
        ...
        if (empty($this->_filterModel[$data['field']])) {
            throw new NotFound('no such field (' . $data['field'] . ') in this filter model');
        }
        ...
        if (isset($definition['custom']) && $definition['custom']) {
            ...
            if (isset($definition['filter'])
                && (
                    $definition['filter'] === ForeignRecords::class
                    || $definition['filter'] === ForeignRecord::class
                    || $definition['filter'] === ForeignId::class
                )
            ) {
                ...
                // equals is also valid (for ForeignId)
    ...
    public function getAclFilters()
    {
        ...
        foreach ($this->_filterObjects as $object) {
            if ($object instanceof AclFilter) {
                $aclFilters[] = $object;
            }
        }
    ...
    public function toArray($_valueToJson = false)
    {
        $result = array();
        foreach ($this->_filterObjects as $filter) {
            if ($filter instanceof FilterGroup && ! $filter instanceof Query) {
                $result[] = array(
    ...
    {
        $this->_options['timezone'] = Core::getUserTimezone();
        $this->setFromArray($_data);
    }
    ...
    public function isFilterSet($_field, $_recursive = false)
    {
        foreach ($this->_filterObjects as $object) {
            ...
            } elseif ($_recursive && $object instanceof FilterGroup) {
    ...
    public function getRequiredColumnsForSelect()
    {
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($result, TRUE));
    ...
    public function hash()
    {
        ...
        foreach ($this->_filterObjects as $object) {
            if ($object instanceof FilterGroup) {
                $data[] = $object->hash();
            } else if ($object instanceof AbstractFilter) {
    ...
    protected function _findFilter($_field, $_getAll = FALSE, $_recursive= FALSE)
    {
        ...
        foreach ($this->_filterObjects as $object) {
            if ($_recursive && $object instanceof FilterGroup) {
    ...
    protected function _removeFilter($_field, $_recursive = FALSE)
    {
        foreach ($this->_filterObjects as $key => $object) {
            ...
            } else if ($object instanceof FilterGroup && $_recursive) {
    ...
    public static function getFilterForModel($_modelOrFilterName, array $_data = array(), $_condition = '', $_options = array())
    {
        ...
        if (! class_exists($modelFilterClass)) {
            ...
            $filter = new FilterGroup(array(), $_condition, $_options);
