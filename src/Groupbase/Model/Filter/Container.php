...
use Fgsl\Groupware\Admin\Model\Config as AdminModelConfig;
use Fgsl\Groupware\Admin\Controller as AdminController;
 ...
 * @subpackage  Filter
 */
class Container extends AbstractFilter implements AclFilter 
{
    /**
    ...
    protected function _setOptions(array $_options)
    {
        if (! isset($_options['applicationName'])) {
            throw new InvalidArgument('Container filter needs the applicationName option');
        }
    ...
    public function setValue($_value)
    {
        ...
        foreach ((array) $_value as $v) {
            ...
            if (strpos($v, '/') === FALSE) {
                try {
                    $container = GroupbaseContainer::getInstance()->getContainerById($v, TRUE);
    ...
    public function appendFilterSql($_select, $_backend)
    {
        ...
        if ($this->_operator === 'notin' || $this->_operator === 'not') {
            $_select->where($this->_getQuotedFieldName($_backend) . ' NOT IN (?)', empty($this->_containerIds) ? new Expression('NULL') : $this->_containerIds);
        } else {
            $_select->where($this->_getQuotedFieldName($_backend) . ' IN (?)', empty($this->_containerIds) ? new Expression('NULL') : $this->_containerIds);
        }
    }
    ...
    public function toArray($_valueToJson = false)
    {
        ...
        if ($_valueToJson == true) {
            ...
            foreach((array) $this->_value as $path) {
                ...
                try {
                    if (($containerId = ModelContainer::pathIsContainer($path))) {
                        $containerData = array_merge($containerData, GroupbaseContainer::getInstance()->getContainerById($containerId, TRUE)->toArray());
                    } else if (($ownerId = ModelContainer::pathIsPersonalNode($path))) {
                        // transform current user
                        $owner = User::getInstance()->getUserByPropertyFromSqlBackend('accountId', $ownerId);
                        $containerData['name'] = $owner->accountDisplayName;
                        $containerData['path'] = "/personal/$ownerId";
                        $containerData['owner'] = $owner->toArray();
                    }
                } catch(\Fgsl\Groupware\Groupbase\Exception\NotFound $e) {
    ...
    protected function _resolve()
    {
        ...
        if (! in_array($this->_operator, array('equals', 'in', 'notin', 'not'))) {
            throw new UnexpectedValue("Operator '{$this->_operator}' not supported.");
        }
    ...
    protected function _resolvePath($_path)
    {
        ...
        if (($containerId = ModelContainer::pathIsContainer($_path))) {
            if ($this->_options['ignoreAcl'] == TRUE) {
                $containerIds[] = $containerId;
            } else if (! is_object(Core::getUser()) || Core::getUser()->hasGrant($containerId, $this->_requiredGrants)) {
                $containerIds[] = $containerId;
            }
        } else if (($ownerId = ModelContainer::pathIsPersonalNode($_path))) {
            $containerIds = $this->_resolveContainerNode('personal', $ownerId);
        } else {
            $node = $_path == '/' ? 'all' : substr($_path, 1);
            $node = $node === 'personal' ? ModelContainer::TYPE_OTHERUSERS : $node;
    ...
    protected function _resolveContainerNode($_node, $_ownerId = NULL)
    {
        ...
        switch ($_node) {
            case 'all':        return GroupbaseContainer::getInstance()->getContainerByACL($currentAccount,
                $appName, $this->_requiredGrants, TRUE, $this->_options['ignoreAcl']);
            case 'personal':   return GroupbaseContainer::getInstance()->getPersonalContainer($currentAccount,
            ...
            case ModelContainer::TYPE_OTHERUSERS:
                return GroupbaseContainer::getInstance()->getOtherUsersContainer($currentAccount, $appName,
            ...
            case 'internal':
                ...
                $adminConfigDefaults = AdminController::getInstance()->getConfigSettings();
                return array($adminConfigDefaults[AdminModelConfig::DEFAULTINTERNALADDRESSBOOK]);
            default:           throw new UnexpectedValue('specialNode ' . $_node . ' not supported.');
        }
    }
    ...
    {
        return GroupbaseContainer::getInstance()->getSharedContainer($currentAccount, $appName, $this->_requiredGrants,
    ...
    protected function _node2path($_operator, $_value)
    {
        switch ($_operator) {
            ...
            default:
                throw new UnexpectedValue("operator '$_operator' not supported");
        }
    }
}
