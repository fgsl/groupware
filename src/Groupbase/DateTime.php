 ...
 */
/**
 * Groupbase\DateTime - Extensions around native php DateTime object
 * 
 * @package     Groupbase
 */
class DateTime extends \DateTime
{
    ...
    public function __call($name, $arguments)
    {
        ...
        throw new InvalidArgument('unknown method: ' . str_replace('php52compat_', '', $name));
    }
...
//            
    ...
    public function compare($_date, $_part = 'c')
    {
        if (! $_date instanceof DateTime) {
            throw new \Fgsl\Groupware\Groupbase\Exception\Date(var_export($_date, TRUE) . ' is not an instance of DateTime');
        }
    ...
    public function hasTime()
    {
        ...
        if (func_num_args() === 1) {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Resetting _hasTime to ' . (int) $paramValue);
    ...
    {
        $thisZD = new DateTime($this->format('U'));
        $datetime2ZD = new DateTime($datetime2->format('U'));
        
        return $datetime2ZD->sub($thisZD, DateTime::TIMESTAMP);
    }
    ...
    public static function isDate($_date)
    {
        if (version_compare(PHP_VERSION, '5.3.0') >= 0) {
            ...
            return Date::isDate($_date, 'yyyy-MM-dd HH:mm:ss');
        }
    }
