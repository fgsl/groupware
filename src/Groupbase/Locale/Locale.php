<?php
namespace Fgsl\Groupware\Groupbase\Locale;

use Fgsl\Groupware\Groupbase\Locale\Data\Translation;
...
class Locale
{
     ...
     * @throws Exception When autodetection has been failed
     */
    ...
    public static function getDefault()
    {
        if ((self::$compatibilityMode === true) or (func_num_args() > 0)) {
            if (!self::$_breakChain) {
                ...
                trigger_error('You are running Locale in compatibility mode... please migrate your scripts', E_USER_NOTICE);
     ...
     * @throws Exception When a autolocale was given
     * @throws Exception When a unknown locale was given
     * @return void
     */
        ...
        foreach ($languages as $locale) {
            ...
            if ($language !== 'C') {
                ...
                $language = str_ireplace(
                    array_keys(Translation::$languageTranslation),
                    array_values(Translation::$languageTranslation),
                    (string) $language
                    );
                
                $language = str_ireplace(
                    array_keys(Translation::$regionTranslation),
                    array_values(Translation::$regionTranslation),
                    $language
                    );
     ...
     *
     * @param string $locale
     * @throws Exception When the given locale is no locale or the autodetection fails
     * @return string
     */
     ...
     *
     * @return CacheInterface The set cache
     */
     ...
     * Sets a cache
     *
     * @param  CacheInterface $cache Cache to set
     * @return void
     */
     ...
     *
     * @param  string|Locale $locale (Optional) Locale to work on
     * @param  boolean            $strict (Optional) Strict preparation
     * @throws Exception When no locale is set which is only possible when the class was wrong extended
     * @return string
     */
    private static function _prepareLocale($locale, $strict = false)
    {
        ...
        // This can only happen when someone extends Locale and erases the default
        if ($locale === null) {
