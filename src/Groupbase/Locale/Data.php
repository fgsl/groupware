...
use Fgsl\Groupware\Groupbase\Xml\Security;
...
class Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            self::$_ldml[(string) $locale] = Security::scanFile($filename);
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            ...
            self::$_cache = new StorageCacheFactory(
                'Core',
                'File',
        ...
        $val = urlencode($val);
        $id  = self::_filterCacheId('LocaleL_' . $locale . '_' . $path . '_' . $val);
        ...
        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($temp), $id, array('Locale'));
            } else {
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        $val = urlencode($val);
        $id  = self::_filterCacheId('LocaleC_' . $locale . '_' . $path . '_' . $val);
        if (!self::$_cacheDisabled && ($result = self::$_cache->load($id))) {
            return unserialize($result);
        }
    ...
    /**
     * Set a cache for Locale_Data
     *
     * @param CacheInterface $cache A cache frontend
     */
