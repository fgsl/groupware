<?php
namespace Fgsl\Groupware\Groupbase\Frontend\Cli;
...
class Cli extends AbstractCli
{
    ...
    public function increaseReplicationMasterId($opts)
    {
        ...
        ModificationLog::getInstance()->increaseReplicationMasterId($count);
    ...
    public function readModifictionLogFromMaster($opts)
    {
        ...
        ModificationLog::getInstance()->readModificationLogFromMaster();
    ...
    */
    ...
    public function forceResync($_opts)
    {
        ...
        if (Core::isLogLevel(LogLevel::INFO))
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' ' . $msg);
    ...
    public function cleanModlog()
    {
        ...
        $deleted = ModificationLog::getInstance()->clean();
    ...
    public function cleanRelations()
    {
        ...
        while ( ($recordSet = $relations->search($filter, $pagination)) && $recordSet->count() > 0 ) {
            ...
            foreach ($models as $model => &$ids) {
                ...
                try {
                    ...
                    if (Core::isLogLevel(LogLevel::INFO))
                        Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' model: ' . $model . ' no application found for it');
                    $doAll = true;
                }
                if (!$doAll) {
                    if ($app instanceof Tinebase_Container)
                    {
                        ...
                        if (!$app instanceof Tinebase_Controller_Record_Abstract) {
                            if (Core::isLogLevel(LogLevel::INFO))
                                Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' model: ' . $model . ' controller: ' . get_class($app) . ' not an instance of Tinebase_Controller_Record_Abstract');
                            continue;
                        }
                    ...
                    if (!$backend instanceof Tinebase_Backend_Interface) {
                        if (Core::isLogLevel(LogLevel::INFO))
                            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' model: ' . $model . ' backend: ' . get_class($backend) . ' not an instance of Tinebase_Backend_Interface');
                        continue;
                    }
                ...
                if ( count($ids) > 0 ) {
                    ...
                    foreach($toDelete as $id) {
                        if ( $recordSet->getById($id)->creation_time && $recordSet->getById($id)->creation_time->isLater($date) ) {
                            Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' relation is about to get deleted that is younger than 1 year: ' . print_r($recordSet->getById($id)->toArray(false), true));
                        }
                    }
        ...
        if (Core::isLogLevel(LogLevel::INFO))
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' ' . $message);
        echo $message . "\n";
    }
    ...
    public function authenticate($_username, $_password)
    {
        ...
        if ($authResult->isValid()) {
            ...
            Core::set('currentAccount', $account);
    ...
    public function executeQueueJob($_opts)
    {
        ...
        if (! $jobId) {
            throw new InvalidArgument('mandatory parameter "jobId" is missing');
        }
        ...
        if (isset($job['account_id'])) {
            Core::set(Core::USER, Tinebase_User::getInstance()->getFullUserById($job['account_id']));
        }
        ...
        do {
            ...
            foreach ($notes as $note) {
                if (!isset($controllers[$note->record_model])) {
                    ...
                    try {
                        $controllers[$note->record_model] = Core::getApplicationInstance($note->record_model);
    ...
    public function cleanCustomfields()
    {
        ...
        foreach($customFieldConfigs as $customFieldConfig) {
            $deleteAll = false;
            try {
                $controller = Core::getApplicationInstance($customFieldConfig->model);
    ...
    protected function _getAllApplicationTables()
    {
        ...
        foreach ($enabledApplications as $application) {
            $result = array_merge($result, Application::getInstance()->getApplicationTables($application));
        }
    ...
    protected function _purgeTables($orderedTables, $where)
    {
        foreach ($orderedTables as $table) {
            ...
            $deleteCount = 0;
            try {
                $deleteCount = Core::getDb()->delete(SQL_TABLE_PREFIX . $table, $where);
            } catch (Zend_Db_Statement_Exception $zdse) {
    ...
    public function addCustomfield(Zend_Console_Getopt $_opts)
    {
        ...
        foreach ($args as $idx => $arg) {
            ...
            if ($key == 'application') {
                $key = 'application_id';
                $value = Application::getInstance()->getApplicationByName($value)->getId();
            }
            $data[$key] = $value;
        }
    ...
    public function setCustomfieldAcl(Zend_Console_Getopt $_opts)
    {
        ...
        if (! isset($data['grants']) || ! isset($data['name'])) {
            throw new InvalidArgument('grants and name params are required');
        }
        ...
        if (! $cf) {
            throw new InvalidArgument('customfield not found');
        }
    ...
    */
    ...
    public function monitoringCheckQueue()
    {
        ...
        if (! $queueConfig->{Tinebase_Config::ACTIONQUEUE_ACTIVE}) {
            ...
            if (! $actionQueue->hasAsyncBackend()) {
                ...
                try {
                    if (null === ($lastDuration = Application::getInstance()->getApplicationState('Tinebase',
                            Application::STATE_ACTION_QUEUE_LAST_DURATION))) {
                        throw new Tinebase_Exception('state ' . Application::STATE_ACTION_QUEUE_LAST_DURATION .
                            ' not set');
                    }
                    if (null === ($lastDurationUpdate = Application::getInstance()->getApplicationState('Tinebase',
                            Application::STATE_ACTION_QUEUE_LAST_DURATION_UPDATE))) {
                        throw new Tinebase_Exception('state ' .
                            Application::STATE_ACTION_QUEUE_LAST_DURATION_UPDATE . ' not set');
                    }
                    ...
                    if (false !== ($currentJobId = $actionQueue->peekJobId())) {
                        if ($currentJobId === ($lastJobId = Application::getInstance()->getApplicationState(
                                'Tinebase', Application::STATE_ACTION_QUEUE_LAST_JOB_ID))) {
                            if (null === ($lastChange = Application::getInstance()->getApplicationState('Tinebase',
                                    Application::STATE_ACTION_QUEUE_LAST_JOB_CHANGE))) {
                                throw new Tinebase_Exception('state ' .
                                    Application::STATE_ACTION_QUEUE_LAST_JOB_CHANGE . ' not set');
                            }
                            ...
                            Application::getInstance()->setApplicationState('Tinebase',
                                Application::STATE_ACTION_QUEUE_LAST_JOB_CHANGE, (string)$now);
                            Application::getInstance()->setApplicationState('Tinebase',
                                Application::STATE_ACTION_QUEUE_LAST_JOB_ID, $currentJobId);
                        }
                    } else {
                        Application::getInstance()->setApplicationState('Tinebase',
                            Application::STATE_ACTION_QUEUE_LAST_JOB_ID, '');
                    }
    ...
    public function undo(Zend_Console_Getopt $opts)
    {
        ...
        $result = ModificationLog::getInstance()->undo($filter, $overwrite, $dryrun, (isset($data['modified_attribute'])?$data['modified_attribute']:null));
        ...
        if (! $dryrun) {
            ...
            foreach ($result['undoneModlogs'] as $modlog) {
                ...
                if (!empty($modifiedAttribute)) {
                    ...
                    if ($modlog->change_type === ModificationLog::CREATED) {
                        echo 'id ' . $modlog->record_id . ' DELETE' . PHP_EOL;
                    } elseif ($modlog->change_type === ModificationLog::DELETED) {
    ...
    public function transferRelations($opts)
    {
        ...
        try {
            ...
            if (Core::isLogLevel(LogLevel::NOTICE)) {
                Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Parameters "oldId", "newId" and "model" are required!');
            }
            exit(1);
        }
        ...
        if (! empty($skippedEntries) && Core::isLogLevel(LogLevel::NOTICE)) {
            Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' ' . count($skippedEntries) . ' entries has been skipped:');
        }
        
        if (Core::isLogLevel(LogLevel::INFO)) {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' The operation has been terminated successfully.');
        }
    ...
    public function userReport()
    {
        ...
        $userStatus = array(
            'total' => array(),
            ModelUser::ACCOUNT_STATUS_ENABLED => array(/* 'showUserNames' => true, 'showClients' => true */),
            ModelUser::ACCOUNT_STATUS_DISABLED => array(),
            ModelUser::ACCOUNT_STATUS_BLOCKED => array(),
            ModelUser::ACCOUNT_STATUS_EXPIRED => array(),
            //'system' => array(),
    ...
    public function reReplicateContainer(Zend_Console_Getopt $opts)
    {
        ...
        $db = Core::getDb();
        $transId = TransactionManager::getInstance()->startTransaction($db);
        ...
        if (count($result) > 0) {
            ...
            $db->query('UPDATE ' . SQL_TABLE_PREFIX . 'timemachine_modlog SET instance_seq = @i:=@i+1, instance_id = "'
                . Core::getTinebaseId() . '" WHERE record_type = "' . $container->model .
        ...
        TransactionManager::getInstance()->commitTransaction($transId);
