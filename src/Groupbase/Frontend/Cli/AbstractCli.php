...
class AbstractCli
{
     ...
     * @return int
     */
    public function updateImportExportDefinition(Getopt $_opts)
    {
        ...
        foreach ($defs as $definitionFilename) {
            ImportExportDefinition::getInstance()->updateOrCreateFromFilename($definitionFilename, $application);
    ...
    public function createContainer(Zend_Console_Getopt $_opts)
    {
        ...
        $app = Application::getInstance()->getApplicationByName($this->_applicationName);
    ...
    protected function _createPhpDemoData($_opts, $checkDependencies)
    {
        ...
        if ($checkDependencies) {
            foreach($className::getRequiredApplications() as $appName) {
                if (Application::getInstance()->isInstalled($appName)) {
        ...
        if (! empty($_requiredKeys)) {
            foreach ($_requiredKeys as $requiredKey) {
                if (! (isset($result[$requiredKey]) || array_key_exists($requiredKey, $result))) {
                    throw new InvalidArgument('Required parameter not found: ' . $requiredKey);
                }
            }
        }
    ...
    protected function _checkAdminRight($exitOnNoPermission = true)
    {
        ...
        if (! Core::getUser()->hasRight($this->_applicationName, Tinebase_Acl_Rights::ADMIN)) {
    ...
    protected function _import($_opts)
    {
        ...
        if ((isset($args['definition']) || array_key_exists('definition', $args)))  {
            if (preg_match("/\.xml/", $args['definition'])) {
                $definition = ImportExportDefinition::getInstance()->getFromFile(
                    $args['definition'],
                    Application::getInstance()->getApplicationByName($this->_applicationName)->getId()
                );
            } else {
                $definition = ImportExportDefinition::getInstance()->getByName($args['definition']);
            }
            ...
            // use generic import
            $definition = ImportExportDefinition::getInstance()->getGenericImport($args['model']);
            $importer = call_user_func($definition->plugin . '::createFromDefinition', $definition, $args);
        } else {
        ...
        foreach ((array) $args['filename'] as $filename) {
            ...
            try {
                ...
                if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' ' . $e->getMessage());
                if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' ' . $e->getTraceAsString());
                continue;
            }
    ...
    protected function _addOutputLogWriter($priority = 5)
    {
        ...
        Core::getLogger()->addWriter($writer);
    }
