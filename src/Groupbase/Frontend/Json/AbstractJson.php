...
use Fgsl\Groupware\Groupbase\Server\Plugin\ImportExportDefinition;
 ...
 */
abstract class AbstractJson extends AbstractFrontend implements JsonInterface
{
    /**
    ...
    public function getRelatableModels()
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ 
            . ' This method is deprecated and will be removed. Please use Tinebase_ModelFactory with ModelConfiguration!');
        
        if (property_exists($this, '_relatableModels') && is_array($this->_relatableModels)) {
            return Relations::getConstraintsConfigs($this->_relatableModels);
        } else {
            return array();
        }
    }
     ...
     */
    public static function resolveContainersAndTags(RecordSet $_records, $modelConfiguration = null, $_resolveProperties = array('container_id', 'tags'))
    {
        $firstRecord = $_records->getFirstRecord();

        if ($firstRecord) {
            if ($firstRecord->has('container_id') && in_array('container_id', $_resolveProperties)) {
                Container::getInstance()->getGrantsOfRecords(
                    $_records,
                    Core::getUser(),
            ...
            if ($firstRecord->has('tags') && in_array('tags', $_resolveProperties) && ! $firstRecord->tags instanceof RecordSet) {
                Tags::getInstance()->getMultipleTagsOfRecords($_records);
            }

        }
    }
    protected function _get($_uid, RecordInterface $_controller)
    {
     ...
     * @param SearchInterface                     $_controller the record controller
     * @param string                              $_filterModel the class name of the filter model to use
     * @param bool|array|Expander                 $_getRelations
     ...
     * @param $filter
     * @param $pagination
     * @param SearchInterface $controller the record controller
    ...
    protected function _longRunningRequest($executionTime = 0)
    {
        ...
        $oldMaxExcecutionTime = Core::setExecutionLifeTime($executionTime);
    ...
    {
        $definition = ImportExportDefinition::getInstance()->get($_importDefinitionId);
        $importer = call_user_func($definition->plugin . '::createFromDefinition', $definition, $_options);

        if (! is_object($importer)) {
            throw new NotFound('No importer found for ' . $definition->name);
        }
     ...
     *
     * @param RecordSet $_records \Fgsl\Groupware\Groupbase\Record\RecordInterface
    ...
    public function getTemplates($nodeId = NULL)
    {
        ...
        try {
            ...
            if (Core::getUser()->hasGrant($node, Tinebase_Model_Grants::GRANT_READ)) {
     ...
     *
     * @param RecordSet $definitions
     * @param Tinebase_Model_Application $application
     * @return array
     */
    protected function _getImportDefinitionRegistryData(RecordSet $definitions = null, Tinebase_Model_Application $application = null)
    {
        if (! $application) {
            $application = Application::getInstance()->getApplicationByName($this->_applicationName);
        }
        if (! $definitions) {
    ...
    protected function _getImportDefinitions(Tinebase_Model_Application $application = null)
    {
        ...
        return ImportExportDefinition::getInstance()->search($filter);
    }

    /**
     * get default definition
     *
     * @param RecordSet $_importDefinitions
     * @param Tinebase_Model_Application $application
    ...
    protected function _getDefaultImportDefinition($_importDefinitions, Tinebase_Model_Application $application = null)
    {
        ...
        try {
            ...
            $defaultDefinition = ImportExportDefinition::getInstance()->getByName($defaultName);
        } catch (NotFound $tenf) {
    ...
    public static function addFilterModelPlugin($formerFilter, $plugin)
    {
        if (class_exists($plugin)) {
            ...
            Core::getLogger()->err(__METHOD__."::".__LINE__.":: Filter model plugin \"$plugin\" doesn't exists");
        }
    }
    ...
    public function __call($method, array $args)
    {
        ...
        if (preg_match('/^(get|save|search|delete|import)([a-z0-9]+)/i', $method, $matches)) {
            ...
            $modelController = Core::getApplicationInstance($this->_applicationName, $model);
