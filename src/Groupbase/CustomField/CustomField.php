 ...
 */
class CustomField implements SearchInterface
{
    use ModlogTrait;
     ...
     * 
     * @var array (app id + modelname => RecordSet with cfs)
     */
    ...
    public function addCustomField(Tinebase_Model_CustomField_Config $_record)
    {
        ...
        $transId = TransactionManager::getInstance()->startTransaction(Core::getDb());
        try {
            try {
            ...
            if ($_record->is_system) {
                $app = Application::getInstance()->getApplicationById($_record->application_id);
     ...
     * @return RecordSet|Tinebase_Model_CustomField_Config of Tinebase_Model_CustomField_Config records
     */
    public function getCustomFieldsForApplication($_applicationId, $_modelName = NULL, $_requiredGrant = Tinebase_Model_CustomField_Grant::GRANT_READ, $_getSystemCFs = false)
    {
        ...
        if (! $result) {
            ...
            if (Core::isLogLevel(LogLevel::TRACE) && (count($result) > 0)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__
    ...
    public function appHasCustomFields($applicationName, $modelName = NULL)
    {
        ...
        $app = Application::getInstance()->getApplicationByName($applicationName);
    ...
    public function saveMultipleCustomFields($_modelName, $_recordIds, $_customFieldValues) 
    {
        ...
        $app = Application::getInstance()->getApplicationByName($app);
    ...
    {
        $applicationId = Application::getInstance()->getApplicationByName($_record->getApplication())->getId();
                ...
                switch (count($filtered)) {
                    case 1:
                        ...
                        if ($customField->valueIsEmpty($value)) {
                            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Deleting cf value for ' . $customField->name);
                            $this->_backendValue->delete($cf);
                        } else {
                            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Updating value for ' . $customField->name . ' to ' . $value);
                            $cf->value = $value;
                    ...
                    case 0:
                        if (! $customField->valueIsEmpty($value)) {
                            ...
                            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Creating value for ' . $customField->name . ' -> ' . $value);
     ...
     *
     * @param Tinebase_Record_Interface $_record
     * @param RecordSet $_customFields
     * @param RecordSet $_configs
     */
    public function resolveRecordCustomFields(Tinebase_Record_Interface $_record, $_customFields = NULL, $_configs = NULL)
    {
        ...
        if ($_configs === NULL) {
            $_configs = $this->getCustomFieldsForApplication(Application::getInstance()->getApplicationByName($_record->getApplication()));
        };
     ...
     * @param RecordSet $configs
     * @param bool $extendedResolving
     */
    protected function _setCfValueInRecord(Tinebase_Record_Interface $record, Tinebase_Model_CustomField_Value $customField, RecordSet $configs, $extendedResolving = false)
    {
     ...
     * 
     * @param  RecordSet $_records     records to get customfields for
     * @param  bool                      $_extendedResolving
     */
    public function resolveMultipleCustomfields(RecordSet $_records, $_extendedResolving = false)
    {
        if (count($_records) == 0) {
            return;
        }
        ...
        $configs = $this->getCustomFieldsForApplication(Application::getInstance()->getApplicationByName($_records->getFirstRecord()->getApplication()));
        ...
        // @see 0007558: reactivate indices in RecordSet
     ...
     * @return RecordSet of Tinebase_Model_CustomField_Value
     */
