...
class Translation
{
    ...
    public static function getAvailableTranslations($appName = 'Tinebase')
    {
        ...
        if ($cache) {
            $cacheId = Helper::convertCacheId(__FUNCTION__ . $appName . sha1(serialize($filesToWatch)));
            $cache = new Zend_Cache_Frontend_File(array(
                'master_files' => $filesToWatch
            ));
            $cache->setBackend(Core::get(Core::CACHE)->getBackend());
    ...
    public static function getLocale($_localeString = 'auto')
    {
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " given localeString '$_localeString'");
        try {
            ...
            if (! in_array($_localeString, $supportedLocales)) {
                if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " '$locale' is not supported, checking fallback");
                ...
                switch ($language) {
                    ...
                    default: 
                        if (in_array($language, $supportedLocales)) {
                            ...
                            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " no suiteable lang fallback found within this locales: " . print_r($supportedLocales, true) );
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " selected locale: '$locale'");
        return $locale;
    }
    ...
    public static function getTranslation($_applicationName, Zend_Locale $_locale = NULL)
    {
        $locale = $_locale
            ?: Core::getLocale()
            ?: (Config::getInstance()->get(Config::DEFAULT_LOCALE, 'en'));
    ...
    public static function getJsTranslations($_locale, $_appName = 'all')
    {
        ...
        if (Core::get(Core::CACHE) && $_appName == 'all') {
     ...
     * @return string
     */
    public static function dateToStringInTzAndLocaleFormat(DateTime $date = null, $timezone = null, $locale = null, $part = 'datetime', $addWeekday = false)
    {
        $date = ($date !== null) ? clone($date) : Tinebase_DateTime::now();
        $timezone = ($timezone !== null) ? $timezone : Core::getUserTimezone();
        $locale = ($locale !== null) ? $locale : Core::get(Core::LOCALE);
        
        $date = new \DateTime::createFromFormat($date->getTimestamp());
        $date->setTimezone($timezone);

        if (in_array($part, array('date', 'time', 'datetime'))) {
            $dateString = $date->toString('Y-m-d H:i:s',$locale);
            if ($addWeekday) {
                $dateString = $date->toString('EEEE', $locale) . ', ' . $dateString;
            }
            $timeString = $date->toString('Y-m-d H:i:s', $locale);
