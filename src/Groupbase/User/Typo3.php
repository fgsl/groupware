    ...
    */
    ...
    public function importUsers($_options = null)
    {
        ...
        foreach($t3users as $user) {
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ .' user: ' . print_r($user->toArray(), true));
            $user->sanitizeAccountPrimaryGroup();
            $user = $this->_sqlUserBackend->addOrUpdateUser($user);
            if (!$user instanceof Tinebase_Model_FullUser) {
                Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Could not add user "' . $user->accountLoginName . '" => Skipping');
                Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' classname ' . get_class($user). ' attributes: ' . print_r($user,1));
                continue;
            }
    ...
    protected function _getUsersFromBackend($_filter, $_accountClass = 'ModelUser')
    {
        $users = new RecordSet($_accountClass);
