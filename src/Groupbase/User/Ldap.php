 ...
 * @subpackage  User
 */
class Ldap extends Sql implements SyncAbleInterface
{
    /**
     * @var array
     */
    ...
    public function __construct(array $_options = array())
    {
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) 
            Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . " Registering " . print_r($this->_options, true));
    ...
    public function registerLdapPlugin(Tinebase_User_Plugin_LdapInterface $plugin)
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) 
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " Registering " . get_class($plugin) . ' LDAP plugin.');
    ...
    public function getUsersFromSyncBackend($_filter = NULL, $_sort = NULL, $_dir = 'ASC', $_start = NULL, $_limit = NULL, $_accountClass = 'ModelUser')
    {
        ...
        $result = new RecordSet($_accountClass);
...
//         }
    ...
    public function setPassword($_userId, $_password, $_encrypt = TRUE, $_mustChange = null)
    {
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . '  $dn: ' . $metaData['dn']);
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . '  $ldapData: ' . print_r($ldapData, true));
    ...
    {
        if ($status == ModelUser::ACCOUNT_STATUS_DISABLED) {
            $ldapData = array(
                'shadowMax'      => 1,
                'shadowInactive' => 1
            );
            ...
            if ($status == ModelUser::ACCOUNT_STATUS_ENABLED) {
    ...
    public function updateUserInSyncBackend(Tinebase_Model_FullUser $_account)
    {
        ...
        if (isset($ldapData[key($rdn)]) && $rdn[key($rdn)] != $ldapData[key($rdn)]) {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . '  rename ldap entry to: ' . $newDN);
    ...
    protected function _getLdapEntry($_property, $_userId)
    {
        switch($_property) {
            case 'accountId':
                $value = $this->_encodeAccountId(ModelUser::convertUserIdToInt($_userId));
                break;
            default:
                /** @noinspection PhpDeprecationInspection */
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) 
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' filter ' . $filter);
        if (Core::isLogLevel(LogLevel::TRACE)) 
            Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' requested attributes ' . print_r($attributes, true));
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) 
            Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' current ldap values ' . print_r($accounts->getFirst(), true));
    ...
    {
        $userId = $this->_encodeAccountId(ModelUser::convertUserIdToInt($_userId));
    ...
    protected function _generateUidNumber()
    {
        ...
        if (isset(Core::getConfig()->samba) && Core::getConfig()->samba->get('manageSAM', FALSE) == true) {
            ...
            $accounts = $this->_ldap->search(
                $filter,
                Core::getConfig()->samba->get('machineDn'),
                Zend_Ldap::SEARCH_SCOPE_SUB,
                array('uidnumber')
            );
        ...
        #if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . "  Existing uidnumbers " . print_r($allUidNumbers, true));
    ...
    protected function _ldap2User(array $_userData, $_accountClass)
    {
                ...
                switch($keyMapping) {
                    ...
                    case 'accountStatus':
                        if ($this->_isUserDisabled($_userData)) {
                            $accountArray[$keyMapping] = ModelUser::ACCOUNT_STATUS_DISABLED;
                        }
                        break;
        ...
        if ($errors) {
            Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Could not instantiate account object for ldap user '
                . print_r($_userData, 1));
            $accountObject = null;
        } else {
        ...
        if ($accountObject instanceof Tinebase_Model_FullUser) {
            if (!isset($accountObject->accountStatus)) {
                $accountObject->accountStatus = ModelUser::ACCOUNT_STATUS_ENABLED;
            }
            if ($accountObject->accountExpires &&
                $accountObject->accountExpires->isEarlier(Tinebase_DateTime::now()) &&
                $accountObject->accountStatus === ModelUser::ACCOUNT_STATUS_ENABLED
            ) {
                $accountObject->accountStatus = ModelUser::ACCOUNT_STATUS_EXPIRED;
            }
        }
...
//            'bday'                  => 'birthdate',
    ...
    protected function _user2ldap(Tinebase_Model_FullUser $_user, array $_ldapEntry = array())
    {
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' LDAP data ' . print_r($ldapData, true));
