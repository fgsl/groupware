 ...
 * 
 * @todo        extend Application_Backend_Sql and replace some functions
 */
...
class Tinebase_User_Sql extends Tinebase_User_Abstract
{
    ...
    public function getUsers($_filter = NULL, $_sort = NULL, $_dir = 'ASC', $_start = NULL, $_limit = NULL, $_accountClass = 'ModelUser')
    {
        ...
        $result = new RecordSet($_accountClass, $rows, TRUE);
    ...
    public function getUserByProperty($_property, $_value, $_accountClass = 'ModelUser')
    {
        ...
        foreach ($this->_sqlPlugins as $plugin) {
            try {
                if ($plugin instanceof Tinebase_User_Plugin_LdapInterface) {
                    throw new InvalidArgument('LDAP plugin ' . get_class($plugin)
        ...
        if ($this instanceof Tinebase_User_Interface_SyncAble) {
            try {
                ...
                if (Core::isLogLevel(LogLevel::CRIT)) Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' user not found in sync backend: ' . $user->getId());
            }
        }
        
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($user->toArray(), true));
     ...
     */
    public function getUserByPropertyFromSqlBackend($_property, $_value, $_accountClass = 'ModelUser')
    {
        if(!(isset($this->rowNameMapping[$_property]) || array_key_exists($_property, $this->rowNameMapping))) {
            throw new InvalidArgument("invalid property $_property requested");
        }
        
        switch($_property) {
            case 'accountId':
                $value = ModelUser::convertUserIdToInt($_value);
                break;
            default:
                $value = $_value;
                break;
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . $select);
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($row, true));
        ...
        try {
            ...
            Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' ' . $e->getMessage() . "\n" .
                "ModelUser::validation_errors: \n" .
     ...
     * @return RecordSet of Tinebase_Model_FullUser
     */
    public function getUsersByPrimaryGroup($groupId)
    {
        ...
        $result = new RecordSet('Tinebase_Model_FullUser', $data, true);
        return $result;
    }
    ...
    {
        $userId = $_userId instanceof ModelUser ? $_userId->getId() : $_userId;
        ...
        $oldPassword = new ModelUserPassword(array('id' => $userId), true);
        $newPassword = new ModelUserPassword($accountData, true);
    ...
    protected function _setPluginsPassword($userId, $password, $encrypt = TRUE)
    {
        foreach ($this->_sqlPlugins as $plugin) {
            try {
                ...
                Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' Could not change plugin password: ' . $e);
    ...
    public function setStatus($_accountId, $_status)
    {
        ...
        switch($_status) {
            case ModelUser::ACCOUNT_STATUS_ENABLED:
            ...
            case ModelUser::ACCOUNT_STATUS_DISABLED:
                $accountData['status'] = $_status;
                break;
                
            case ModelUser::ACCOUNT_STATUS_EXPIRED:
            ...
            default:
                throw new InvalidArgument('$_status can be only enabled, disabled or expired');
                break;
        }
    ...
    */
    ...
    public function updateUserInSqlBackend(Tinebase_Model_FullUser $_user)
    {
        ...
        $accountId = ModelUser::convertUserIdToInt($_user);
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($accountData, true));
    ...
    public function addUserInSqlBackend(Tinebase_Model_FullUser $_user)
    {
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Adding user to SQL backend: ' . $_user->accountLoginName);
    ...
    public function directDeleteUserInSqlBackend($_userId)
    {
        ...
        try {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' error while deleting account ' . $e->__toString());
            TransactionManager::getInstance()->rollBack();
            throw($e);
        }
    ...
    public function deleteUserInSqlBackend($_userId)
    {
        ...
        try {
            ...
            $accountsTable->update(array(   'is_deleted' => 1,
                                            'status' => ModelUser::ACCOUNT_STATUS_DISABLED,
                                            'visibility' => ModelUser::VISIBILITY_HIDDEN), $where);
    ...
    public function deleteAllUsers()
    {
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Deleting ' . count($users) .' users');
     ...
     * @param  string  $_accountClass  type of model to return
     * @return RecordSet of 'ModelUser' or 'Tinebase_Model_FullUser'
     */
    public function getMultiple($_id, $_accountClass = 'Tinebase_Model_FullUser') 
    {
        if (empty($_id)) {
            return new RecordSet($_accountClass);
        }
        ...
        $result = new RecordSet($_accountClass, $queryResult, TRUE);
    ...
    public function sendDeactivationNotification($accountId)
    {
        ...
        try {
            ...
            $view->tine20Url            = Core::getHostname();
    ...
    public function countNonSystemUsers()
    {
        ...
        $select = $select = $this->_db->select()
            ...
            ->where($this->_db->quoteInto($this->_db->quoteIdentifier('status') . ' != ?', ModelUser::ACCOUNT_STATUS_DISABLED))
