...
class Tinebase_User_ActiveDirectory extends Tinebase_User_Ldap
{
    ...
    public function setExpiryDateInSyncBackend($_accountId, $_expiryDate)
    {
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . " {$metaData['dn']}  $ldapData: " . print_r($ldapData, true));
    ...
    public function updateUserInSyncBackend(Tinebase_Model_FullUser $_account)
    {
        ...
        if ($rdn['CN'] != $ldapData['cn']) {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG))
                Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . '  rename ldap entry to: ' . $newDN);
        ...
        if (Core::isLogLevel(LogLevel::DEBUG))
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . '  $dn: ' . $ldapEntry['dn']);
        if (Core::isLogLevel(LogLevel::TRACE))
            Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . '  $ldapData: ' . print_r($ldapData, true));
    ...
    protected function _ldap2User(array $_userData, $_accountClass = 'Tinebase_Model_FullUser')
    {
        ...
        if ($accountArray['accountExpires'] instanceof Tinebase_DateTime && Tinebase_DateTime::now()->compare($accountArray['accountExpires']) == -1) {
            $accountArray['accountStatus'] = ModelUser::ACCOUNT_STATUS_EXPIRED;
        }
        ...
        if ($errors) {
            Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Could not instantiate account object for ldap user ' . print_r($_userData, 1));
            $accountObject = null;
        } else {
    ...
    protected function _user2ldap(Tinebase_Model_FullUser $_user, array $_ldapEntry = array())
    {
        ...
        foreach ($_user as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'accountStatus':
                    if ($value === ModelUser::ACCOUNT_STATUS_ENABLED) {
                        // unset account disable flag
                        $ldapData['useraccountcontrol'] &= ~self::ACCOUNTDISABLE;
                    } elseif ($value === ModelUser::ACCOUNT_STATUS_DISABLED) {
        ...
        if (Core::isLogLevel(LogLevel::TRACE))
            Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' LDAP data ' . print_r($ldapData, true));
