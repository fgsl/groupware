...
use Fgsl\Groupware\Groupbase\ModelConfiguration\NewModelConfiguration;
...
/**
 * New abstract implementation of RecordInterface
 *
 * @package     Groupbase
 * @subpackage  Record
 */
class NewAbstract extends ConfigConst implements RecordInterface
{
    /**
     * holds the configuration object (must be declared in the concrete class)
     *
     * @var NewModelConfiguration
     */
    ...
    public function __construct($_data = null, $_bypassFilters = false, $_convertDates = true)
    {
        ...
        if (true !== $_convertDates) {
            throw new InvalidArgument(static::class . ' doesnt support convertDates anymore');
        }
    ...
    public static function getConfiguration()
    {
        if (null === static::$_configurationObject) {
            ...
            static::$_configurationObject = new NewModelConfiguration($modelConfiguration);
        }
    ...
    public function __set($_name, $_value)
    {
        if (! isset(static::$_configurationObject->_fields[$_name])) {
            throw new UnexpectedValue($_name . ' is no property of ' . static::class);
        }
     ...
     * @todo remove custom fields handling (use RecordSet for them)
     */
    public function setFromArray(array &$_data)
    {
        ...
        if (isset(static::$_configurationObject->_fields['customfields'])) {
            $application = Application::getInstance()->getApplicationByName($this->getApplication());
            $customFields = CustomField::getInstance()->getCustomFieldsForApplication($application,
                static::class)->name;
            $recordCustomFields = [];
        } else {
            $customFields = [];
            ...
            if (isset($_data[$fieldName]) && is_array($_data[$fieldName])) {
                ...
                if (in_array($config['type'], ['record', 'records']) && isset($config['config']['appName']) &&
                    ...
                    $this->{$fieldName} = $config['type'] == 'record' ?
                        ...
                        new RecordSet($modelName, $_data[$fieldName], $this->bypassFilters, true);
                }
            }
        }
    ...
    public function diff($_record, $omitFields = array())
    {
        ...
        if (! $_record instanceof RecordInterface) {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__
                . ' Did not get RecordInterface, diffing against empty record');
            $model = static::class;
        ...
        foreach (array_keys(static::$_configurationObject->getFields()) as $fieldName) {
            ...
            if (in_array($fieldName, static::$_configurationObject->datetimeFields)) {
                ...
                /** @var RecordInterface $emptyRecord */
    ...
    public function getPathPart(RecordInterface $_parent = null, RecordInterface $_child = null)
    {
        ...
        $delegate = Core::getDelegate($this->getApplication(), 'getPathPartDelegate_' . get_called_class() ,
            'Fgsl\Groupware\Groupbase\Record\AbstractGetPathPartDelegatorInterface');
        if (false !== $delegate) {
    ...
    {
        throw new NotImplemented(static::class . ' doesn\'t implement translate()');
    }
        ...
        if ($_recursive) {
            foreach ($this->_data as $property => $propValue) {
                if ($propValue && is_object($propValue) &&
                    ($propValue instanceof RecordInterface ||
    ...
    {
        if ($_bool !== true) {
            throw new NotImplemented(static::class . ' does not support convertDates anymore');
        }
        return true;
    }
}