...
use Fgsl\Groupware\Groupbase\Model\InputFilter\CrlfConvert;
...
use Fgsl\Groupware\Groupbase\Exception\Date;
...
/**
 * Abstract implemetation of RecordInterface
 * 
 * @package     Groupbase
 * @subpackage  Record
 */
abstract class AbstractRecord extends ConfigConst implements RecordInterface
{
    /**
    ...
    public function __construct($_data = NULL, $_bypassFilters = false, $_convertDates = true)
    {
        ...
        if ($this->_identifier === NULL) {
            throw new DefinitionFailure('$_identifier is not declared');
        }
        ...
        if ($this->has('description') && (! (isset($this->_filters['description']) || array_key_exists('description', $this->_filters)))) {
            $this->_filters['description'] = new CrlfConvert();
        }
    ...
    public static function getConfiguration()
    {
        ...
        if (! static::$_configurationObject) {
            static::$_configurationObject = new ModelConfiguration(static::$_modelConfiguration);
        }
    ...
    public static function resetConfiguration()
    {
        ...
        ModelConfiguration::resetAvailableApps();
    }
    ...
    public function setFromArray(array &$_data)
    {
        ...
        if ($this->has('customfields')) {
            $application = Application::getInstance()->getApplicationByName($this->_application);
            $customFields = CustomField::getInstance()->getCustomFieldsForApplication($application, get_class($this))->name;
                ...
                if (isset($_data[$fieldName]) && is_array($_data[$fieldName])) {
                    ...
                    if (in_array($config['type'], ['record', 'records']) && isset($config['config']['appName']) &&
                        ...
                        $this->{$fieldName} = $config['type'] == 'record' ?
                            ...
                            new RecordSet($modelName, $_data[$fieldName], $this->bypassFilters,
                                $this->convertDates);
                    }
                }
            }
    ...
    public function setFromJsonInUsersTimezone(&$_data)
    {
        ...
        date_default_timezone_set(Core::getUserTimezone());

        // NOTE: setFromArray creates new DateTimes of $this->datetimeFields
        ...
        foreach ($this->_datetimeFields as $field) {
            ...
            foreach ($toConvert as $convertField => &$value) {
                if (! method_exists($value, 'setTimezone')) {
                    throw new Validation($convertField . ' must be a method setTimezone');
                } 
        ...
        if ($_recursive) {
            foreach ($this->_properties as $property => $propValue) {
                if ($propValue && is_object($propValue) &&
                        (in_array('RecordInterface', class_implements($propValue)) ||
                            $propValue instanceof RecordSet) ) {
    ...
    public function toArray($_recursive = TRUE)
    {
        ...
        if ($this->convertDates === true) {
            if (! is_string($this->dateConversionFormat)) {
                $this->_convertDateTimeToString($recordArray, AbstractRecord::ISO8601LONG);
            } else {
    ...
    public function isValid($_throwExceptionOnInvalidData = false)
    {
        ...
        if ($_throwExceptionOnInvalidData) {
            $e = new Validation('Some fields ' . implode(',', array_keys($inputFilter->getMessages()))
    ...
    public function __set($_name, $_value)
    {
        if (! (isset($this->_validators[$_name]) || array_key_exists ($_name, $this->_validators))) {
            throw new UnexpectedValue($_name . ' is no property of $this->_properties');
        }
    ...
    protected function _validateField($name, $value)
    {
        ...
        $e = new Validation('the field ' . implode(',', array_keys($inputFilter->getMessages())) . ' has invalid content');
    ...
    */
    ...
    protected function _getFilter($field = null)
    {
        ...
        if (! (isset(self::$_inputFilters[$keyName]) || array_key_exists($keyName, self::$_inputFilters))) {
            if ($field !== null) {
                ...
                self::$_inputFilters[$keyName] = new Input($filters, $validators);
            } else {
                self::$_inputFilters[$keyName] = new Input($this->_filters, $this->_validators);
            }
    ...
    /**
     * Converts DateTimes into custom representation
     *
    ...
    /**
     * Converts iso8601 formated dates into DateTime representation
     * 
     * @param array &$_data
     * @return void
     */
                ...
                try {
                    if (is_array($value)) {
                        foreach($value as $dataKey => $dataValue) {
                            ...
                            $value[$dataKey] =  (int)$dataValue == 0 ? NULL : new DateTime($dataValue);
                        }
                    } else {
                        $value = (int)$value == 0 ? NULL : new DateTime($value);
                        
                    }
                } catch (Date $zde) {
                    Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Error while converting date field "' . $field . '": ' . $zde->getMessage());
                    $value = NULL;
                }
     ...
     * DateTime to create dates in the user timezone. otherwise they will be 
     * created with Etc/GMT+<offset> as timezone which would lead to incorrect 
     * results in datetime computations!
     * 
     * @param  string DateTime::ISO8601 representation of a datetime filed
    ...
    {
        throw new NotAllowed('Unsetting of properties is not allowed');
    }
    ...
    /**
     * converts a int, string or RecordInterface to a id
     *
     * @param int|string|RecordInterface $_id the id to convert
     * @param string $_modelName
     * @return int|string
     * @throws InvalidArgument
     */
    public static function convertId($_id, $_modelName = 'AbstractRecord')
    {
        if ($_id instanceof $_modelName) {
            /** @var RecordInterface $_id */
            if (! $_id->getId()) {
                throw new InvalidArgument('No id set!');
            }
            $id = $_id->getId();
        } elseif (is_array($_id)) {
            throw new InvalidArgument('Id can not be an array!');
        } else {
            $id = $_id;
        }
    
        if ($id === 0) {
            throw new InvalidArgument($_modelName . '.id can not be 0!');
        }
    ...
    /**
     * returns a Diff record with differences to the given record
     * 
     * @param RecordInterface $_record record for comparison
    ...
    public function diff($_record, $omitFields = array())
    {
        ...
        if (! $_record instanceof RecordInterface) {
            if (!empty($_record)) {
                return $_record;
            }
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__
                . ' Did not get AbstractRecord, diffing against empty record');
        ...
        foreach (array_keys($this->_validators) as $fieldName) {
            ...
            if (in_array($fieldName, $this->_datetimeFields)) {
                if ($ownField instanceof DateTime
                    ...
                    /** @var DateTime $recordField */
                    
                    if (! $ownField instanceof DateTime) {
                        if (Core::isLogLevel(LogLevel::NOTICE)) Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . 
                            ' Convert ' . $fieldName .' to DateTime to make sure we have the compare() method');
                        $ownField = new DateTime($ownField);
                    }
                ...
                continue;
            } elseif ($ownField instanceof RecordInterface || $ownField instanceof RecordSet) {
                if ($ownField instanceof RecordInterface && is_scalar($recordField)) {
            ...
            } elseif (empty($ownField) && $recordField instanceof RecordInterface) {
                ...
                continue;
            } elseif (empty($ownField) && $recordField instanceof RecordSet) {
                $emptyRecordSet = new RecordSet($recordField->getRecordClassName(), array());
                ...
                continue;
            } elseif ($ownField instanceof FilterGroup || $recordField instanceof FilterGroup) {
            ...
            } elseif ($recordField instanceof RecordInterface && is_scalar($ownField)) {
                // maybe we have the id of the record -> just compare the id
                ...
                continue;
            } elseif ((empty($ownField)    && $recordField instanceof RecordSet && count($recordField) == 0)
                ||     (empty($recordField) && $ownField    instanceof RecordSet && count($ownField) == 0) )
            {
                continue;
            }
     ...
     * @throws InvalidArgument
     */
    public function isObsoletedBy(RecordInterface $_record)
    {
        if (get_class($_record) !== get_class($this)) {
            throw new InvalidArgument('Records could not be compared');
        } else if ($this->getId() && $_record->getId() !== $this->getId()) {
            throw new InvalidArgument('Record id mismatch');
        }
    ...
    public function translate()
    {
        ...
        if (!empty($this->_toTranslate)) {
            $translate = Translation::getTranslation($this->_application);
    ...
    {
        $translation = Translation::getTranslation(preg_replace('/_.*/', '', static::class), $locale);
        ...
        foreach((array)($diff->oldData) as $property => $oldValue)
        {
            if ('customfields' === $property) {
                ...
                if (!in_array($property, ['notes', 'relations'])) {
                    $oldValue = new RecordSet($model, $oldValue);
                }
            }
    ...
    public function applyDiff(Diff $diff)
    {
        ...
        foreach((array)($diff->diff) as $property => $oldValue)
        {
            if (is_array($oldValue) && count($oldValue) === 4 &&
                ...
                // RecordSetDiff
                $recordSetDiff = new RecordSetDiff($oldValue);

                if (! $this->$property instanceof RecordSet) {
                    $this->$property = new RecordSet($oldValue['model'],
    ...
    {
        /** @var AbstractRecord_GetPathPartDelegatorInterface $delegate */
        $delegate = Core::getDelegate($this->_application, 'getPathPartDelegate_' . get_called_class() ,
                                                'AbstractRecord_GetPathPartDelegatorInterface');
        if (false !== $delegate) {
    ...
    public function getPathNeighbours()
    {
        ...
        $result = array(
            'parents'  => $relations->getRelationsOfRecordByDegree($this, Relation::DEGREE_PARENT, true)->asArray(),
            'children' => $relations->getRelationsOfRecordByDegree($this, Relation::DEGREE_CHILD, true)->asArray()
        );
    ...
    public function getIdFromProperty($_property, $_getIdFromRecord = true)
    {
        ...
        if (is_object($value) && $value instanceof RecordInterface) {
        ...
        throw new UnexpectedValue($_property . '\'s value is neither a record nor an id value');
    }
