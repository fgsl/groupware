...
use Fgsl\Groupware\Groupbase\Container\ContainerInterface;
use Fgsl\Groupware\Groupbase\Controller\AlarmInterface;
    ...
    AlarmInterface
{
    ...
    public function getApplicationBasePath($_application, $_type = null)
    {
        $application = $_application instanceof Tinebase_Model_Application
            ? $_application
            : Application::getInstance()->getApplicationById($_application);
     ...
     * @return RecordSet of Tinebase_Model_Tree_Node
     */
        ...
        switch ($options['tine20']['mode']) {
            ...
            case 'xb':
                ...
                try {
                    ...
                    Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Writing to file : ' .
            ...
            default:
                Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Got mode : ' .
    ...
    protected function _updateFileObject(Tinebase_Model_Tree_Node $_parentNode, $_id, $_hash, $_hashFile = null, $_avResult = null)
    {
        ...
        if (isset($_SERVER['HTTP_X_OC_MTIME'])) {
            ...
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " using X-OC-MTIME: {$updatedFileObject->last_modified_time->format(Tinebase_Record_Abstract::ISO8601LONG)} for {$updatedFileObject->id}");
    ...
    public function getEffectiveAndLocalQuota(Tinebase_Model_Tree_Node $node)
    {
        ...
        $personalNode = null;
        if (Application::getInstance()->isInstalled('Filemanager')) {
            $personalNode = $this->stat('/Filemanager/folders/personal');
        }

        /** @var Tinebase_Model_Application $tinebaseApplication */
        $tinebaseApplication = Application::getInstance()->getApplicationByName('Tinebase');
        ...
        foreach ($this->_getTreeNodeBackend()->getAllFolderNodes(new RecordSet(
    ...
    /**
     * @param RecordSet $_nodes
     * @param int $_sizeDiff
     * @param int $_revisionSizeDiff
     * @throws Tinebase_Exception_Record_NotAllowed
     */
    protected function _updateFolderSizesUpToRoot(RecordSet $_nodes, $_sizeDiff, $_revisionSizeDiff)
    {
        ...
        $applicationController->setApplicationState($tinebaseApplication,
            Application::STATE_FILESYSTEM_ROOT_SIZE, $rootSize);
        $applicationController->setApplicationState($tinebaseApplication,
            Application::STATE_FILESYSTEM_ROOT_REVISION_SIZE, $rootRevisionSize);
    ...
    {
        Application::getInstance()->getApplicationState('Tinebase',
            Application::STATE_FILESYSTEM_ROOT_SIZE, true);
    }
    ...
    public function rename($oldPath, $newPath)
    {
        ...
        $transactionId = $transactionManager->startTransaction(Core::getDb());
        ...
        try {
            ...
            if (dirname($oldPath) != dirname($newPath)) {
                ...
                $this->_updateFolderSizesUpToRoot(new RecordSet('Tinebase_Model_Tree_Node', array($oldParent)),
                    0 - (int)$node->size, 0 - (int)$node->revision_size);
                $this->_updateFolderSizesUpToRoot(new RecordSet('Tinebase_Model_Tree_Node', array($newParent)),
...
if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__
        ...
        // update hash of all parent folders
        $this->_updateDirectoryNodesHash($path);TransactionManager::getInstance()->commitTransaction($transactionId);
            $transactionId = null;
        } finally {
            if (null !== $transactionId) {
                TransactionManager::getInstance()->rollBack();
    ...
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) 
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Removing directory ' . $path);
        ...
        try {
            ...
            if (count($children) > 0) {
                if ($recursive !== true) {
                    throw new InvalidArgument('directory not empty');
                } else {
            ...
            if (false === $recursion) {
                $this->_updateFolderSizesUpToRoot(new RecordSet('Tinebase_Model_Tree_Node', array($node)),
                    0 - (int)$node->size, 0 - (int)$node->revision_size);
            }
    ...
    public function stat($path, $revision = null, $getDeleted = false)
    {
        ...
        try {
            ...
            if (strlen($pathParts[0]) !== 40 && !ctype_digit($pathParts[0])) {
                $oldPart = $pathParts[0];
                $pathParts[0] = Application::getInstance()->getApplicationByName($pathParts[0])->getId();
    ...
    public function unDeleteFileNode($id, $updateDirectoryNodesHash = true)
    {
        ...
        try {
            ...
            ModificationLog::setRecordMetaData($node, 'undelete', $node);
            ...
            if ($object->is_deleted) {
                ModificationLog::setRecordMetaData($object, 'undelete', $object);
                $object->indexed_hash = '';
    ...
    {
        if ($node->type === Tinebase_Model_Tree_FileObject::TYPE_FOLDER) {
            throw new InvalidArgument('can not unlink directories');
        }
    ...
    protected function _createDirectoryTreeNode($_parentId, $name)
    {
        ...
        try {
            ...
            if (null !== ($deletedNode = $this->_getTreeNodeBackend()->getChild($parentId, $name, true, false)) &&
                ...
                if (null === $parentNode) {
                    try {
                        $appId = Application::getInstance()->getApplicationById($name)->getId();
                        if ($appId !== $name) {
                            throw new InvalidArgument('path needs to start with /appId/folders/...');
                        }
                    } catch (Tinebase_Exception_NotFound $tenf) {
                        throw new InvalidArgument('path needs to start with /appId/folders/...');
                    }
                ...
                ModificationLog::setRecordMetaData($directoryObject, 'create');
        ...
        try {
            ...
            if (null !== ($deletedNode = $this->_getTreeNodeBackend()->getChild($parentId, $_name, true, false)) &&
                ...
                if (isset($_SERVER['HTTP_X_OC_MTIME'])) {
                    ...
                    Tinebase_Server_WebDAV::getResponse()->setHeader('X-OC-MTime', 'accepted');
                    if (Core::isLogLevel(LogLevel::DEBUG)) {
                        Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " using X-OC-MTIME: {$object->last_modified_time->format(Tinebase_Record_Abstract::ISO8601LONG)} for {$_name}");
                    }
                }
                ...
                ModificationLog::setRecordMetaData($fileObject, 'create');
                ...
                if (isset($_SERVER['HTTP_X_OC_MTIME'])) {
                    ...
                    Tinebase_Server_WebDAV::getResponse()->setHeader('X-OC-MTime', 'accepted');
                    if (Core::isLogLevel(LogLevel::DEBUG)) {
                        Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " using X-OC-MTIME: {$fileObject->last_modified_time->format(Tinebase_Record_Abstract::ISO8601LONG)} for {$_name}");
                    }
    ...
    public function createFileBlob($contents)
    {
        ...
        if (!file_exists($hashDirectory)) {
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' create hash directory: ' . $hashDirectory);
        ...
        if (!file_exists($hashFile)) {
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' create hash file: ' . $hashFile);
            rewind($handle);
     ...
     * 
     * @param string|Tinebase_Model_Tree_Node|RecordSet  $nodeId
     * @return RecordSet of Tinebase_Model_Tree_Node
     *
    ...
    public function getTreeNodeChildren($nodeId, $getDeleted = false)
    {
        if ($nodeId instanceof Tinebase_Model_Tree_Node) {
            $nodeId = $nodeId->getId();
            $operator = 'equals';
        } elseif ($nodeId instanceof RecordSet) {
    ...
    */
    ...
    public function update(Tinebase_Model_Tree_Node $_node)
    {
        ...
        try {
            ...
            ModificationLog::setRecordMetaData($_node, 'update', $currentNodeObject);
            ModificationLog::setRecordMetaData($fileObject, 'update', $fileObject);
            ...
            if (isset($_SERVER['HTTP_X_OC_MTIME'])) {
                ...
                Tinebase_Server_WebDAV::getResponse()->setHeader('X-OC-MTime', 'accepted');
                if (Core::isLogLevel(LogLevel::DEBUG))
                    Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " using X-OC-MTIME: {$fileObject->last_modified_time->format(Tinebase_Record_Abstract::ISO8601LONG)} for {$_node->name}");
    ...
    public function getPathOfNode($node, $getPathAsString = false)
    {
        ...
        try {
            ...
            $nodesPath = new RecordSet('Tinebase_Model_Tree_Node', array($node));
    ...
    protected function _getPathNodes($path)
    {
        ...
        if (empty($pathParts)) {
            throw new InvalidArgument('empty path provided');
        }
        
        $subPath   = null;
        $pathNodes = new RecordSet('Tinebase_Model_Tree_Node');
            ...
            case Tinebase_FileSystem::FOLDER_TYPE_PERSONAL:
                if ($_path->containerOwner && ($_topLevelAllowed || ! $_path->isToplevelPath())) {
                    if ($_path->isToplevelPath()) {
                        $hasPermission = ($_path->containerOwner === Core::getUser()->accountLoginName || $_action === 'get');
                    } else {
    ...
    {
        if (Core::getUser()->hasGrant($_node, Tinebase_Model_Grants::GRANT_ADMIN, 'Tinebase_Model_Tree_Node')) {
            return true;
        }
        ...
        $result = Core::getUser()->hasGrant($_node, $requiredGrant, 'Tinebase_Model_Tree_Node');
    ...
    {
        $result = new RecordSet('Tinebase_Model_Tree_Node');
        $accountId = ModelUser::convertUserIdToInt($_accountId);
        $appAndModel = Application::extractAppAndModel($_recordClass);
        $app = Application::getInstance()->getApplicationByName($appAndModel['appName']);
    ...
    public function getPersonalContainer($_accountId, $_recordClass, $_owner, $_grant = Tinebase_Model_Grants::GRANT_READ, $_ignoreACL = false)
    {
        ...
        if (count($result) === 0 && $accountId === $ownerId) {
            /** @noinspection PhpUndefinedMethodInspection */
            $account = (!$_accountId instanceof ModelUser)
    ...
    public function getGrantsOfContainer($_containerId, $_ignoreAcl = false, /** @noinspection PhpUnusedParameterInspection */$_grantModel = 'Tinebase_Model_Grants')
    {
        ...
        if (! $_ignoreAcl) {
            if (! Core::getUser()->hasGrant($record, Tinebase_Model_Grants::GRANT_READ)) {
    ...
    public function reportPreviewStatus()
    {
        ...
        if (false === $this->_previewActive) {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' previews are disabled');
            return $status;
        }
    ...
    {
        Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' starting to sanitize previews');
    ...
    public function sendCRUDNotification(array $_accountIds, array $_crudActions)
    {
        ...
        foreach($_accountIds as $accountId) {
            $locale = Tinebase_Translation::getLocale(Core::getPreference()->getValueForUser(Tinebase_Preference::LOCALE, $accountId));
    ...
    public function notifyQuota()
    {
        ...
        if ($total > 0) {
            ...
            if ($quotaIncludesRevisions) {
                $totalUsage = intval(Application::getInstance()->getApplicationState($tinebaseApplication,
                    Application::STATE_FILESYSTEM_ROOT_REVISION_SIZE));
            } else {
                $totalUsage = intval(Application::getInstance()->getApplicationState($tinebaseApplication,
                    Application::STATE_FILESYSTEM_ROOT_SIZE));
        ...
        $personalNode = null;
        $notifiedNodes = [];
        if ($totalByUser > 0 && Application::getInstance()->isInstalled('Filemanager')) {
            $personalNode = $this->stat('/Filemanager/folders/personal');
            /** @var Tinebase_Model_Tree_Node $node */
        ...
        foreach ($imapBackend->getAllEmailUsers() as $emailUser) {
            ...
            if (true === $alert) {
                ...
                foreach (Tinebase_User::getInstance()->getMultiple(array_unique(array_merge(
                    ...
                    $locale = Tinebase_Translation::getLocale(Core::getPreference()->getValueForUser(Tinebase_Preference::LOCALE,
                        $user->accountId));
