...
use Fgsl\Groupware\Groupbase\ModelConfiguration\ConfigConst as TMCC;
use Fgsl\Groupware\Groupbase\Controller\Event;
use Fgsl\Groupware\Groupbase\Controller\SearchInterface;
...
use Fgsl\Groupware\Groupbase\Controller\AbstractRecord as AbstractControllerRecord;
...
use Fgsl\Groupware\Mail\MessageFileLocation;
...
abstract class AbstractRecord
    extends Event
    implements RecordInterface, SearchInterface
{
    use ModlogTrait;
    ...
    protected $_getMultipleGrant = ModelGrants::GRANT_READ;
    protected $_requiredFilterACLget = [ModelGrants::GRANT_READ, ModelGrants::GRANT_ADMIN];
    protected $_requiredFilterACLupdate  = [ModelGrants::GRANT_EDIT, ModelGrants::GRANT_ADMIN];
    protected $_requiredFilterACLsync  = [ModelGrants::GRANT_SYNC, ModelGrants::GRANT_ADMIN];
    protected $_requiredFilterACLexport  = [ModelGrants::GRANT_EXPORT, ModelGrants::GRANT_ADMIN];

    /**
     * returns controller for records of given model
     *
     * @param string $_model
     * @return Controller|AbstractController|AbstractControllerRecord
     */
    public static function getController($_model)
    {
        list($appName, /*$i*/, $modelName) = explode('_', $_model);
        return Core::getApplicationInstance($appName, $modelName);
    }
    ...
    public function assertPublicUsage()
    {
        ...
        if (!$currentUser) {
            Core::set(Core::USER, User::getInstance()
                ->getFullUserByLoginName(User::SYSTEM_USER_ANONYMOUS));
        }
        ...
        return function () use ($oldvalues) {
            ...
            if ($oldvalues['currentUser']) {
                Core::set(Core::USER, $oldvalues['currentUser']);
            }
        };
    }
     ...
     * @param boolean|array|Expander $_getRelations
     * @param boolean $_onlyIds
     * @param string $_action for right/acl check
     * @return Tinebase_Record_RecordSet|array
     */
    public function search(FilterGroup $_filter = NULL, Pagination $_pagination = NULL, $_getRelations = FALSE, $_onlyIds = FALSE, $_action = self::ACTION_GET)
    {
        if (! $_filter) {
            $_filter = FilterGroup::getFilterForModel($this->_modelName);
        }
        ...
        if (! $_onlyIds) {
            if ($_getRelations instanceof Expander) {
     ...
     * @return int|array
     */
    public function searchCount(FilterGroup $_filter, $_action = self::ACTION_GET)
    {
        $this->_checkRight($_action);
        $this->checkFilterACL($_filter, $_action);
        ...
        if (! $_id) { // yes, we mean 0, null, false, ''
            ...
            if ($this->_doContainerACLChecks) {
                if ($_containerId === NULL) {
                    $containers = Container::getInstance()->getPersonalContainer(Core::getUser(), $this->_applicationName, Core::getUser(), ModelGrants::GRANT_ADD);
    ...
    protected function _getRelatedData($record)
    {
        ...
        try {
            ...
            if ($record->has('attachments') && Core::isFilesystemAvailable()) {
    ...
    public function getMultiple($_ids, $_ignoreACL = FALSE, Expander $_expander = null)
    {
        ...
        $containerIds = ($this->_doContainerACLChecks && $_ignoreACL !== TRUE)
           ? Container::getInstance()->getContainerByACL(
               Core::getUser(),
    ...
    public function create(RecordInterface $_record, $_duplicateCheck = true)
    {
        ...
        $db = (method_exists($this->_backend, 'getAdapter')) ? $this->_backend->getAdapter() : Core::getDb();

        if ($_record->has('attachments') && isset($_record->attachments) && Core::isFilesystemAvailable()) {
        ...
        try {
            ...
            if ($this->sendNotifications()) {
                $this->doSendNotifications($createdRecord, Core::getUser(), 'created');
            }
        ...
        if ($this->_clearCustomFieldsCache) {
            Core::getCache()->clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('customfields'));
        }
     ...
     */
    protected function _setContainer(RecordInterface $_record)
    {
     ...
     */
    protected function _setAutoincrementValues(RecordInterface $_record, RecordInterface $_oldRecord = null)
    {
     ...
     */
    protected function _duplicateCheck(RecordInterface $_record)
    {
        ...
        $duplicates = $this->search($duplicateFilter, new Pagination(array('limit' => 5)), /* $_getRelations = */ true);
     ...
     */
    protected function _getDuplicateFilter(RecordInterface $_record)
    {
        ...
        $filter = FilterGroup::getFilterForModel($this->_modelName, $filterData);
     ...
     *
     * @param   RecordInterface $createedRecord   the just updated record
     ...
     * @param string $action
     */
    protected function _increaseContainerContentSequence(RecordInterface $record, $action = NULL)
    {
        if ($record->has('container_id')) {
            Container::getInstance()->increaseContentSequence($record->container_id, $action, $record->getId());
        }
    }
     ...
     *  
     * @param RecordInterface $_record
     * @param RecordInterface $_origRecord
     * @param string $_action
     * @return  void
     */
    protected function _forceModlogInfo(RecordInterface $_record, RecordInterface $_origRecord, $_action = NULL)
    {
        if ($this->_doForceModlogInfo && ! empty($_origRecord)) {
            // on create
            if ($_action == self::ACTION_CREATE) {
                if (! empty($_origRecord->created_by)) {
                    if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Force modlog - created_by: ' . $_origRecord->created_by);
                    $_record->created_by = $_origRecord->created_by;
                }
                if (! empty($_origRecord->creation_time)) {
                    if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Force modlog - creation_time: ' . $_origRecord->creation_time);
                    $_record->creation_time = $_origRecord->creation_time;
                }
                if (! empty($_origRecord->last_modified_by)) {
                    if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Force modlog - last_modified_by: ' . $_origRecord->last_modified_by);
                    $_record->last_modified_by = $_origRecord->last_modified_by;
                }
                if (! empty($_origRecord->last_modified_time)) {
                    if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Force modlog - last_modified_time: ' . $_origRecord->last_modified_time);
     ...
     */
    public function update(RecordInterface $_record, $_duplicateCheck = TRUE)
    {
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' '
...
//             }
            ...
            if ($this->_sendNotifications && count($currentMods) > 0) {
                $this->doSendNotifications($updatedRecordWithRelatedData, Core::getUser(), 'changed', $currentRecord);
            }
     ...
     * 
     * @param RecordInterface $_record
     * @param RecordInterface $_currentRecord
     */
    ...
    protected function _setRelatedData(RecordInterface $updatedRecord, RecordInterface $record, RecordInterface $currentRecord = null, $returnUpdatedRelatedData = false, $isCreate = false)
    {
        ...
        if ($record->has('attachments') && isset($record->attachments) && Core::isFilesystemAvailable()) {
     ...
     * @param RecordInterface|null $currentRecord
     * @return bool
     */
    protected function _checkRelationsForPathGeneratingModels(RecordInterface $newRecord, RecordInterface $currentRecord = null)
    {
    ...
    protected function _setSystemNotes($_updatedRecord, $_systemNoteType = Tinebase_Model_Note::SYSTEM_NOTE_NAME_CREATED, $_currentMods = NULL)
    {
        ...
        Tinebase_Notes::getInstance()->addSystemNote($_updatedRecord, Core::getUser(), $_systemNoteType, $_currentMods);
    }
     ...
     * 
     * @param FilterGroup|array $_filterOrIds
    ...
    {
        $ids = ($_filterOrIds instanceof FilterGroup) ? $this->search($_filterOrIds, NULL, FALSE, TRUE, self::ACTION_UPDATE) : $_filterOrIds;
        ...
        if ($this->_omitModLog !== TRUE && is_object(Core::getUser())) {
            if (Core::isLogLevel(LogLevel::INFO)) Core::getLogger()->info(__METHOD__ . '::' . __LINE__
    ...
    protected function _handleRelationsOnUpdateMultiple($key, $value)
    {
        ...
        if (Core::isLogLevel(LogLevel::DEBUG)) {
            Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' New relations: ' . print_r($this->_newRelations, true)
    ...
    public function updateMultiple($_filter, $_data)
    {
        ...
        if (Core::isLogLevel(LogLevel::INFO)) {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Updated ' . $this->_updateMultipleResult['totalcount'] . ' records.');
        }
    ...
    public function processUpdateMultipleIteration($_records, $_data)
    {
        ...
        $bypassFilters = FALSE;
        /** @var RecordInterface $currentRecord */
     ...
     *
     * @param  array|RecordInterface|Tinebase_Record_RecordSet $_ids array of record identifiers
     * @return Tinebase_Record_RecordSet
     * @throws Exception
     */
    public function delete($_ids)
    {
        ...
        if (count((array)$ids) != count($records)) {
            Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Only ' . count($records) . ' of ' . count((array)$ids) . ' records exist.');
        }
        ...
        try {
            ...
            if ($this->sendNotifications()) {
                foreach ($records as $record) {
                    $this->doSendNotifications($record, Core::getUser(), 'deleted');
                }
            }
     ...
     * @return  Tinebase_Record_RecordSet
     */
    public function deleteByFilter(FilterGroup $_filter)
    {
        $oldMaxExcecutionTime = ini_get('max_execution_time');

        Core::setExecutionLifeTime(300); // 5 minutes

        $ids = $this->search($_filter, NULL, FALSE, TRUE);
        $deletedRecords = $this->delete($ids);
        
        // reset max execution time to old value
        Core::setExecutionLifeTime($oldMaxExcecutionTime);
    ...
    public function move($_records, $_target, $_containerProperty = 'container_id')
    {
        ...
        if ($this->_doContainerACLChecks) {
            ...
            if (! Core::getUser()->hasGrant($targetContainerId, ModelGrants::GRANT_ADD)) {
                Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Permission denied to add records to container.');
            ...
            $containerIdsWithDeleteGrant = Container::getInstance()->getContainerByACL(Core::getUser(), $this->_applicationName, ModelGrants::GRANT_DELETE, TRUE);
            if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__
        ...
        $filter = FilterGroup::getFilterForModel($filterClass, [
     ...
     * @throws Tinebase_Exception_AccessDenied
     */
    public function unDelete(RecordInterface $_record)
    {
        ...
        $transactionId = Tinebase_TransactionManager::getInstance()->startTransaction(Core::getDb());
        try {
     ...
     * @throws Tinebase_Exception_AccessDenied
     */
    protected function _deleteRecord(RecordInterface $_record)
    {
        $this->_checkGrant($_record, self::ACTION_DELETE);
     ...
     * @param RecordInterface $_record
     */
    protected function _deleteLinkedObjects(RecordInterface $_record)
    {
        ...
        if ($_record->has('attachments') && Core::isFilesystemAvailable()) {
     ...
     * @param RecordInterface $_record
     */
    protected function _unDeleteLinkedObjects(RecordInterface $_record)
    {
        ...
        if ($_record->has('attachments') && count($_record->attachments) > 0 && Core::isFilesystemAvailable()) {
     ...
     */
    public function deleteLinkedRelations(RecordInterface $record, $modelsToDelete = array(), $typesToDelete = array())
    {
        ...
        Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Deleting all '
        ...
        foreach ($relations as $relation) {
            if (in_array($relation->related_model, $modelsToDelete) || in_array($relation->type, $typesToDelete)) {
                ...
                $appController = Core::getApplicationInstance($appName, $itemName);

                try {
                    $appController->delete($relation->related_id);
                } catch (Exception $e) {
                    Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Error deleting: ' . $e->getMessage());
                }
            }
        }
    }
     ...
     * @param RecordInterface $_oldRecord
     * @return boolean
     * @throws Tinebase_Exception_AccessDenied
     *
        ...
        /** @noinspection PhpUnusedParameterInspection */ $_oldRecord = NULL)
    {
        ...
        if (Core::getUser()->hasGrant($_record->container_id, ModelGrants::GRANT_ADMIN)) {
            return TRUE;
        }
        ...
        switch ($_action) {
            case self::ACTION_GET:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, ModelGrants::GRANT_READ);
                break;
            case self::ACTION_CREATE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, ModelGrants::GRANT_ADD);
                break;
            case self::ACTION_UPDATE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, ModelGrants::GRANT_EDIT);
                break;
            case self::ACTION_DELETE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, ModelGrants::GRANT_DELETE);
                break;
        }

        if (! $hasGrant) {
            Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' No permissions to ' . $_action . ' in container ' . $_record->container_id);
            if ($_throw) {
     ...
     */
    public function checkFilterACL(FilterGroup $_filter, $_action = self::ACTION_GET)
    {
        if (! $this->_doContainerACLChecks) {
            if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ 
     ...
     * @return void
     */
    protected function _saveAlarms(RecordInterface $_record)
    {
        ...
        foreach ($_record->alarms as $alarm) {
            try {
                ...
                Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' ' . $teia->getMessage());
            }
        }
     ...
     * @throws Tinebase_Exception_InvalidArgument
     */
    protected function _inspectAlarmSet(RecordInterface $_record, Tinebase_Model_Alarm $_alarm)
    {
     ...
     *
     * @param  RecordInterface|Tinebase_Record_RecordSet $_record
     */
    public function getAlarms($_record)
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . " Resolving alarms and add them to record set.");
     ...
     * @return void
     */
    protected function _inspectAlarmGet(RecordInterface $_record)
    {
     ...
     * @param Pagination $_pagination (only valid if $_mixed instanceof FilterGroup)
     * @return Tinebase_Record_RecordSet
     */
    protected function _convertToRecordSet($_mixed, $_refresh = FALSE, Pagination $_pagination = NULL)
    {
        if ($_mixed instanceof FilterGroup) {
            // FILTER (FilterGroup)
        ...
        } elseif ($_mixed instanceof RecordInterface) {
     ...
     */
    protected function _createDependentRecords(RecordInterface $_createdRecord, RecordInterface $_record, $_property, $_fieldConfig)
    {
        ...
        if ($_record->has($_property) && $_record->{$_property}) {
            ...
            /** @var ControllerInterface $ccn */
            $ccn = $_fieldConfig['controllerClassName'];
            /** @var RecordInterface $controller */
            $controller = $ccn::getInstance();

            /** @var RecordInterface $rec */
            ...
            if (Core::isLogLevel(LogLevel::INFO)) {
                Core::getLogger()->info(__METHOD__ . '::' . __LINE__. ' Creating ' . $_record->{$_property}->count() . ' dependent records on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName);
            }
     ...
     * @throws Tinebase_Exception_Record_NotAllowed
     */
    protected function _updateDependentRecords(RecordInterface $_record, /** @noinspection PhpUnusedParameterInspection */
                                               RecordInterface $_oldRecord, $_property, $_fieldConfig)
    {
        if (! (isset($_fieldConfig['dependentRecords']) || array_key_exists('dependentRecords', $_fieldConfig)) || ! $_fieldConfig['dependentRecords']) {
            return;
        }
        ...
        if (($_record->{$_property} === NULL) || (! $_record->has($_property))) {
            if (Core::isLogLevel(LogLevel::INFO)) {
                Core::getLogger()->info(__METHOD__ . '::' . __LINE__. ' Skip updating dependent records (got NULL) on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName . ' with id = "' . $_record->getId() . '"');
            }
            return;
        }
        ...
        /** @var ControllerInterface $ccn */
        $ccn = $_fieldConfig['controllerClassName'];
        /** @var RecordInterface|SearchInterface $controller */
        $controller = $ccn::getInstance();
        $recordClassName = $_fieldConfig['recordClassName'];
        $filterClassName = $_fieldConfig['filterClassName'];
        /** @var Tinebase_Record_RecordSet|RecordInterface $existing */
        ...
        if (! empty($_record->{$_property}) && $_record->{$_property} && $_record->{$_property}->count() > 0) {
            ...
            foreach ($_record->{$_property} as $record) {
                ...
                if (!empty($record->getId())) {
                    try {
                        ...
                        if (!empty($prevRecord->diff($record)->diff)) {
                            if (Core::isLogLevel(LogLevel::INFO)) {
                                Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Updating dependent record with id = "' . $record->getId() . '" on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName);
                            }
                ...
                if (true === $create) {
                    ...
                    if (Core::isLogLevel(LogLevel::INFO)) {
                        Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Creating dependent record with id = "' . $crc->getId() . '" on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName);
                    }
                }
            }
        }
        ...
        $filter = FilterGroup::getFilterForModel($filterClassName, $filterArray, 'AND',
        ...
        if (! empty($deleteIds)) {
            if (Core::isLogLevel(LogLevel::INFO)) {
                Core::getLogger()->info(__METHOD__ . '::' . __LINE__. ' Deleting dependent records with id = "' . print_r($deleteIds, 1) . '" on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName);
            }
...
HumanResources_CliTests.testSetContractsEndDate */
        ...
        if (Core::isLogLevel(LogLevel::INFO) && $unDeleteRecords->count() > 0) {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__. ' undeleting dependent records with id = "' . print_r($unDeleteRecords->getArrayOfIds(), 1) . '" on property ' . $_property . ' for ' . $this->_applicationName . ' ' . $this->_modelName);
        }
     ...
     */
    protected function _getPathsOfRecord(RecordInterface $record, $depth = false)
    {
        ...
        foreach ($parentRelations as $parent) {
            ...
            if (!is_object($parent->related_record)) {
                $parent->related_record = Core::getApplicationInstance($parent->related_model)->get($parent->related_id);
            }
    ...
    public function doSendNotifications(/** @noinspection PhpUnusedParameterInspection */
        RecordInterface $_record, Tinebase_Model_FullUser $_updater, $_action, RecordInterface $_oldRecord = NULL, array $_additionalData = array())
    {
    ...
    public function deleteContainerContents(Tinebase_Model_Container $_container, $_ignoreAcl = FALSE, $_filter = null)
    {
        ...
        if (! in_array($model, array('Filemanager_Model_Node')) &&
            ...
            if (null === $_filter) {
                /** @var FilterGroup $_filter */
                $_filter = FilterGroup::getFilterForModel(
                    $model,
                    [],
     ...
     * @return Tinebase_Record_RecordSet
     */
    public function getResolvedGroupRecords(RecordInterface $record, $foreignModel, $groupField, $idProp)
    {
     ...
     *
     * @param MessageFileLocation $location
     * @param Felamimail_Model_Message $message
     * @returns RecordInterface|null
     */
    public function fileMessage(MessageFileLocation $location, Felamimail_Model_Message $message)
    {
