...
use Fgsl\Groupware\Groupbase\Controller\Event as ControllerEvent;
use Fgsl\Groupware\Groupbase\Expressive\RouteHandler;
 ...
 * @subpackage  Server
 */
class Controller extends ControllerEvent
{
    ...
    {
        $this->_writeAccessLog = Application::getInstance()->isInstalled('Tinebase')
            && (Core::get('serverclassname') !== 'ActiveSync_Server_Http' 
                || (Application::getInstance()->isInstalled('ActiveSync')
    ...
    public function login($loginName, $password, \Zend\Http\PhpEnvironment\Request $request, $clientIdString = NULL)
    {
        ...
        Core::getLogger()->addReplacement($password);
        ...
        if (Config::getInstance()->{Config::PASSWORD_NTLMV2_HASH_UPDATE_ON_LOGIN}) {
        ...
        try {
            ...
            if ($accountsController instanceof Tinebase_User_Interface_SyncAble) {
                ...
                try {
                    ...
                    Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' Failed to sync user data for: ' . $_username . ' reason: ' . $e->getMessage());
            ...
            if (Core::isLogLevel(LogLevel::CRIT)) Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' Account ' . $_username . ' not found in account storage.');
            $_accessLog->result = Tinebase_Auth::FAILURE_IDENTITY_NOT_FOUND;
        } catch (Zend_Db_Adapter_Exception $zdae) {
            if (Core::isLogLevel(LogLevel::CRIT)) Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' Some database connection failed: ' . $zdae->getMessage());
    ...
    public function initUser(Tinebase_Model_FullUser $_user, $fixCookieHeader = true)
    {
        ...
        Core::setupUserLocale('auto');
        
        // need to set userTimeZone again
        $userTimezone = Core::getPreference()->getValue(Tinebase_Preference::TIMEZONE);
        Core::setupUserTimezone($userTimezone);
    }
    ...
    protected function _initUserSession($fixCookieHeader = true)
    {
        ...
        if (Config::getInstance()->get(Config::SESSIONUSERAGENTVALIDATION, TRUE)) {
            Tinebase_Session::registerValidatorHttpUserAgent();
        } else {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' User agent validation disabled.');
        }
        
        // we only need to activate ip session validation for non-encrypted connections
        $ipSessionValidationDefault = Core::isHttpsRequest() ? FALSE : TRUE;
        if (Config::getInstance()->get(Config::SESSIONIPVALIDATION, $ipSessionValidationDefault)) {
            Tinebase_Session::registerValidatorIpAddress();
        } else {
            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Session ip validation disabled.');
        }
        ...
        Tinebase_Session::getSessionNamespace()->currentAccount = Core::getUser();
    }
    ...
    protected function _loginFailed($authResult, Tinebase_Model_AccessLog $accessLog)
    {
        ...
        if (null !== $user) {
            ...
            $warnLoginFailures = Config::getInstance()->get(Config::WARN_LOGIN_FAILURES, 4);
            if ($user->loginFailures >= $warnLoginFailures) {
                $loglevel = LogLevel::WARN;
            }
        }

        if (Core::isLogLevel($loglevel)) Core::getLogger()->log(
            __METHOD__ . '::' . __LINE__
    ...
    {
        $result = $this->login($loginName, $password, Core::get(Core::REQUEST), $clientIdString);
    ...
    {
        if ($_pwType === 'password' && ! Config::getInstance()->get(Config::PASSWORD_CHANGE, TRUE)) {
        ...
        if ($_pwType === 'password') {
            if (!Tinebase_Auth::getInstance()->isValidPassword($loginName, $_oldPassword)) {
                throw new InvalidArgument('Old password is wrong.');
            }
    ...
    {
        $allowedRoleChanges = Config::getInstance()->get(Config::ROLE_CHANGE_ALLOWED);
        
        if (!$allowedRoleChanges) {
            throw new Tinebase_Exception_AccessDenied('It is not allowed to switch to this account');
        }
        
        $currentAccountName = Core::getUser()->accountLoginName;
    ...
    public function logout()
    {
        if ($this->_writeAccessLog) {
            if (Core::isRegistered(Core::USER) && is_object(Core::getUser())) {
    ...
    public function getImage($application, $identifier, $location = '')
    {
        if ($location === 'vfs') {
            ...
            $appController = Core::getApplicationInstance($application);
    ...
    protected function _updateAccessLog(Tinebase_Model_FullUser $user, Tinebase_Model_AccessLog $accessLog)
    {
        ...
        Core::set(Core::USERACCESSLOG, $accessLog);
    }
    ...
    protected function _updateCredentialCache($loginName, $password)
    {
        ...
        Core::set(Core::USERCREDENTIALCACHE, $credentialCache);
    }
    ...
    public function rebuildPaths()
    {
        if (true !== Config::getInstance()->featureEnabled(Config::FEATURE_SEARCH_PATH)) {
            Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' search paths are not enabled');
            return false;
        }
        ...
        foreach($applications as $application) {
            try {
                $app = Core::getApplicationInstance($application, '', true);
            } catch (Tinebase_Exception_NotFound $tenf) {
                continue;
            }

            if (! $app instanceof Controller_Abstract) {
                continue;
            }
            ...
            foreach($pathModels as $pathModel) {
                $controller = Core::getApplicationInstance($pathModel, '', true);
                ...
                if (Core::isLogLevel(LogLevel::INFO)) {
                    ...
                    Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Build paths for ' . $result['totalcount'] . ' records of ' . $pathModel);
                }
            }
        }
     ...
     */
    public function rebuildPathsIteration(RecordSet $records)
    {
        ...
        foreach ($records as $record) {
            try {
                ...
                Core::getLogger()->crit(__METHOD__ . '::' . __LINE__ . ' record path building failed: '
    ...
    public static function addFastRoutes(
        ...
        $r->addGroup('', function (\FastRoute\RouteCollector $routeCollector) {
            $routeCollector->get('/favicon[/{size}[/{ext}]]', (new RouteHandler(
                Controller::class, 'getFavicon', [
                RouteHandler::IS_PUBLIC => true
            ]))->toArray());
        });

        $r->addGroup('/Tinebase', function (\FastRoute\RouteCollector $routeCollector) {
            $routeCollector->get('/_status[/{apiKey}]', (new RouteHandler(
                Controller::class, 'getStatus', [
                RouteHandler::IS_PUBLIC => true
            ]))->toArray());
        });
    ...
    {
        if (! Config::getInstance()->get(Config::STATUS_INFO) || ! Config::getInstance()->get(Config::STATUS_API_KEY)) {
            return new \Zend\Diactoros\Response\EmptyResponse();
        }
        
        if ($apiKey !== Config::getInstance()->get(Config::STATUS_API_KEY, false)) {
    ...
    {
        if ($size == 'svg' || $ext == 'svg') {
            $config = Config::getInstance()->get(Config::BRANDING_FAVICON_SVG);
        ...
        $imageBlob = Core::getCache()->load($cacheId);

        if (! $imageBlob) {
            $config = Config::getInstance()->get(Config::BRANDING_FAVICON);
            ...
            if (Core::isLogLevel(LogLevel::DEBUG))
                Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Using favicon ' . $icon);
            ...
            Core::getCache()->save($imageBlob, $cacheId);
        }
    ...
    {
        if (Config::getInstance()->{Config::ACTIONQUEUE}->{Config::ACTIONQUEUE_ACTIVE} &&
    ...
    public function measureActionQueue($start)
    {
        ...
        $now = time();
        $lastUpdate = Application::getInstance()->getApplicationState('Tinebase',
            Application::STATE_ACTION_QUEUE_LAST_DURATION_UPDATE);
        if ($now - intval($lastUpdate) > 58) {
            Application::getInstance()->setApplicationState('Tinebase',
                Application::STATE_ACTION_QUEUE_LAST_DURATION, sprintf('%.3f', $duration));
            Application::getInstance()->setApplicationState('Tinebase',
                Application::STATE_ACTION_QUEUE_LAST_DURATION_UPDATE, $now);
        }
    }
        ...
        foreach ($contentClasses as $cc) {
            switch ($cc) {
                case self::SYNC_CLASS_CONTACTS:
                    $filter = [
                        ['field' => 'application_id', 'operator' => 'equals', 'value' =>
                            Application::getInstance()->getApplicationByName('Addressbook')->getId()],
                ...
                case self::SYNC_CLASS_EVENTS:
                    $filter = [
                        ['field' => 'application_id', 'operator' => 'equals', 'value' =>
                            Application::getInstance()->getApplicationByName('Calendar')->getId()],
                ...
                case self::SYNC_CLASS_TASKS:
                    $filter = [
                        ['field' => 'application_id', 'operator' => 'equals', 'value' =>
                            Application::getInstance()->getApplicationByName('Tasks')->getId()],
