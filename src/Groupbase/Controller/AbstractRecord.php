...
use Fgsl\Groupware\Groupbase\Controller\Record\RecordInterface;
    ...
    implements RecordInterface, SearchInterface
{
    ...
    public function assertPublicUsage()
    {
        ...
        if (!$currentUser) {
            Core::set(Core::USER, Tinebase_User::getInstance()
        ...
        if (! $_id) { // yes, we mean 0, null, false, ''
            ...
            if ($this->_doContainerACLChecks) {
                if ($_containerId === NULL) {
                    $containers = Tinebase_Container::getInstance()->getPersonalContainer(Core::getUser(), $this->_applicationName, Core::getUser(), Tinebase_Model_Grants::GRANT_ADD);
        ...
        try {
            ...
            if ($this->resolveCustomfields()) {
                $cfConfigs = Tinebase_CustomField::getInstance()->getCustomFieldsForApplication(
                    Application::getInstance()->getApplicationByName($this->_applicationName));
    ...
    public function create(Tinebase_Record_Interface $_record, $_duplicateCheck = true)
    {
        ...
        try {
            $transactionId = TransactionManager::getInstance()->startTransaction($db);
            ...
            if ($_record->has('created_by')) {
                ...
                ModificationLog::setRecordMetaData($_record, self::ACTION_CREATE);
    ...
    protected function _getRelationDuplicateFilter($record)
    {
        ...
        if (! $relations instanceof RecordSet) {
            $relations = new RecordSet('Tinebase_Model_Relation', $relations, /* $_bypassFilters = */ true);
        }
...
//             }
    ...
    protected function _concurrencyManagement($_record, $_currentRecord)
    {
        ...
        $modLog = ModificationLog::getInstance();
        $modLog->manageConcurrentUpdates(
            Application::getInstance()->getApplicationByName($this->_applicationName)->getId(),
    ...
    protected function _setRelatedData(Tinebase_Record_Interface $updatedRecord, Tinebase_Record_Interface $record, Tinebase_Record_Interface $currentRecord = null, $returnUpdatedRelatedData = false, $isCreate = false)
    {
        ...
        if ($record->has('relations') && isset($record->relations)
            && (is_array($record->relations) || $record->relations instanceof RecordSet))
        {
        ...
        if ($record->has('tags') && isset($record->tags) && (is_array($record->tags) || $record->tags instanceof RecordSet)) {
    ...
    public function concurrencyManagementAndModlogMultiple($_filterOrIds, $_oldData, $_newData)
    {
        ...
        if ($this->_omitModLog !== TRUE && is_object(Core::getUser())) {
            ...
            $currentMods = ModificationLog::getInstance()->writeModLogMultiple($ids, $_oldData, $_newData, $this->_modelName, $this->_getBackendType(), $updateMetaData);
            /** @noinspection PhpUndefinedVariableInspection */
    ...
    public function updateMultiple($_filter, $_data)
    {
        ...
        $this->_updateMultipleResult = array(
            'results'           => new RecordSet($this->_modelName),
            'exceptions'        => new RecordSet('Tinebase_Model_UpdateMultipleException'),
            'totalcount'        => 0,
            'failcount'         => 0,
        );
    ...
    {
        if(! $currentRecord->relations || get_class($currentRecord->relations) != 'RecordSet') {
            $currentRecord->relations = new RecordSet('Tinebase_Model_Relation');
        }
     ...
     *
     * @param  array|Tinebase_Record_Interface|RecordSet $_ids array of record identifiers
     * @return RecordSet
     * @throws Exception
     */
    public function delete($_ids)
    {
        if ($_ids instanceof $this->_modelName || $_ids instanceof RecordSet) {
    ...
    public function move($_records, $_target, $_containerProperty = 'container_id')
    {
        ...
        if ($this->_doContainerACLChecks) {
            ...
            if (! Core::getUser()->hasGrant($targetContainerId, Tinebase_Model_Grants::GRANT_ADD)) {
            ...
            $containerIdsWithDeleteGrant = Tinebase_Container::getInstance()->getContainerByACL(Core::getUser(), $this->_applicationName, Tinebase_Model_Grants::GRANT_DELETE, TRUE);
            if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__
    ...
    public function unDelete(Tinebase_Record_Interface $_record)
    {
        if ($this->_purgeRecords && !$_record->has('created_by')) {
            throw new InvalidArgument('record of type ' . get_class($_record) . ' can\'t be undeleted');
        }
        ...
        try {
            ...
            ModificationLog::setRecordMetaData($updateRecord, 'undelete', $updateRecord);
    ...
    protected function _deleteRecord(Tinebase_Record_Interface $_record)
    {
        ...
        if (! $this->_purgeRecords && $_record->has('created_by')) {
            ModificationLog::setRecordMetaData($_record, self::ACTION_DELETE, $_record);
    ...
    {
        $relations = isset($record->relations) && $record->relations instanceof RecordSet
            ? $record->relations
        ...
        /** @noinspection PhpUnusedParameterInspection */ $_oldRecord = NULL)
    {
        ...
        if (Core::getUser()->hasGrant($_record->container_id, Tinebase_Model_Grants::GRANT_ADMIN)) {
            return TRUE;
        }
        ...
        switch ($_action) {
            case self::ACTION_GET:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, Tinebase_Model_Grants::GRANT_READ);
                break;
            case self::ACTION_CREATE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, Tinebase_Model_Grants::GRANT_ADD);
                break;
            case self::ACTION_UPDATE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, Tinebase_Model_Grants::GRANT_EDIT);
                break;
            case self::ACTION_DELETE:
                $hasGrant = Core::getUser()->hasGrant($_record->container_id, Tinebase_Model_Grants::GRANT_DELETE);
                break;
        }
    ...
    {
        if (! $_record->alarms instanceof RecordSet) {
            $_record->alarms = new RecordSet(Tinebase_Model_Alarm::class,
                is_array($_record->alarms) ? $_record->alarms : [], true);
        }
        $alarms = new RecordSet(Tinebase_Model_Alarm::class);
    ...
    protected function _inspectAlarmSet(Tinebase_Record_Interface $_record, Tinebase_Model_Alarm $_alarm)
    {
        if (! $_record->{$this->_recordAlarmField} instanceof DateTime) {
            throw new InvalidArgument('alarm reference time is not set');
        }
     ...
     *
     * @param  Tinebase_Record_Interface|RecordSet $_record
     */
    public function getAlarms($_record)
    {
        ...
        $records = $_record instanceof RecordSet ? $_record : new RecordSet($this->_modelName, array($_record));
        ...
        foreach ($alarms as $alarm) {
            ...
            if (!isset($record->alarms)) {
                $record->alarms = new RecordSet('Tinebase_Model_Alarm');
            }
    ...
    protected function _convertToRecordSet($_mixed, $_refresh = FALSE, Tinebase_Model_Pagination $_pagination = NULL)
    {
        if ($_mixed instanceof Tinebase_Model_Filter_FilterGroup) {
        ...
        } elseif ($_mixed instanceof RecordSet) {
            // RECORDSET (RecordSet)
            ...
            if ($_refresh) {
                ...
                $result = new RecordSet(get_class($_mixed), array($_mixed));
            }
            ...
            $result = new RecordSet($this->_modelName);
        }
    ...
    protected function _createDependentRecords(Tinebase_Record_Interface $_createdRecord, Tinebase_Record_Interface $_record, $_property, $_fieldConfig)
    {
        ...
        if ($_record->has($_property) && $_record->{$_property}) {
            ...
            $new = new RecordSet($recordClassName);
            ...
            if (is_array($_record->{$_property})) {
                $rs = new RecordSet($recordClassName);
                                               ...
                                               Tinebase_Record_Interface $_oldRecord, $_property, $_fieldConfig)
    {
        ...
        /** @var RecordSet|Tinebase_Record_Interface $existing */
        $existing = new RecordSet($recordClassName);
...
HumanResources_CliTests.testSetContractsEndDate */
    ...
    protected function _getPathsOfRecord(Tinebase_Record_Interface $record, $depth = false)
    {
        ...
        $result = new RecordSet('Tinebase_Model_Path');
    ...
    public function getResolvedGroupRecords(Tinebase_Record_Interface $record, $foreignModel, $groupField, $idProp)
    {
        ...
        $result = new RecordSet($foreignModel);
