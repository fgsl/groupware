 ...
 */
class Preference extends AbstractPreference
{
    ...
    public function getApplicationPreferenceDefaults($_preferenceName, $_accountId=NULL, $_accountType=Tinebase_Acl_Rights::ACCOUNT_TYPE_USER)
    {
        ...
        switch($_preferenceName) {
            ...
            case self::GRID_STRIPE_ROWS:
                ...
                $preference->options    = '<?xml version="1.0" encoding="UTF-8"?>
                    <options>
                        <special>' . Preference_Abstract::YES_NO_OPTIONS . '</special>
                    </options>';
                  $preference->personal_only = FALSE;
                break;
            ...
            case self::ADVANCED_SEARCH:
                ...
                break;
            default:
                throw new NotFound('Default preference with name ' . $_preferenceName . ' not found.');
        }
    ...
    protected function _getSpecialOptions($_value)
    {
        ...
        switch ($_value) {
            ...
            case Preference::DEFAULT_APP:
                $applications = Application::getInstance()->getApplications();
                foreach ($applications as $app) {
                    if (
                    $app->status == 'enabled'
                    && $app->name != 'Tinebase'
                    && Core::getUser()->hasRight($app->name, Tinebase_Acl_Rights_Abstract::RUN)
                    ) {
    ...
    public function doSpecialJsonFrontendActions(Tinebase_Frontend_Json_Abstract $_jsonFrontend, $name, $value, $appName)
    {
        if ($appName == $this->_application) {
            ...
            if ($value == Tinebase_Model_Preference::DEFAULT_VALUE) {
                $value = $this->{$name};
                if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Using default (' . $value . ') for ' . $name);
            }
            
            $session = Core::get(Tinebase_Session::SESSION);
