...
use Fgsl\Groupware\Groupbase\Controller\Record\AbstractRecord;
 ...
 */
abstract class AbstractPreference extends AbstractSql
{
    /**
     * yes no options
     *
     ...
     */
    abstract public function getApplicationPreferenceDefaults($_preferenceName, $_accountId = NULL, $_accountType = Rights::ACCOUNT_TYPE_USER);
     ...
     * @return RecordSet|array of preferences / pref ids
     */
    public function search(Tinebase_Model_Filter_FilterGroup $_filter = NULL, Tinebase_Model_Pagination $_pagination = NULL, $_onlyIds = FALSE)
    {
        ...
        if (! $_filter->isFilterSet('account')) {
            $accountFilter = $_filter->createFilter('account', 'equals', array(
                ...
                'accountType' => Rights::ACCOUNT_TYPE_USER
            ));
            ...
            if ($accountFilterValue['accountId'] != $userId && $accountFilterValue['accountType'] == Rights::ACCOUNT_TYPE_USER) {
                if (!Tinebase_Acl_Roles::getInstance()->hasRight($this->_application, Core::getUser()->getId(), Rights_Abstract::ADMIN)) {
                    return new RecordSet('Tinebase_Model_Preference');
                }
            }
        }
        
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($_filter->toArray(), TRUE));
        ...
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r((is_array($allPrefs)) ? $allPrefs : $allPrefs->toArray(), TRUE));
     ...
     * 
     * @param RecordSet $_prefs
     * @param Tinebase_Model_Filter_FilterGroup $_filter
     */
    protected function _addDefaultAndRemoveUndefinedPrefs(RecordSet $_prefs, Tinebase_Model_Filter_FilterGroup $_filter)
    {
        ...
        if (count($undefinedPrefs) > 0) {
            ...
            foreach ($undefinedPrefs as $undefinedPrefName) {
                ...
                if (Core::isLogLevel(LogLevel::INFO)) Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Removed undefined preference from result: ' . $undefinedPrefName);
            }
        }
    }
    ...
    {
        return (is_object(Core::getUser())) ? Core::getUser()->getId() : '0';
    }
     ...
     */
    public function getValueForUser($_preferenceName, $_accountId, $_accountType = Rights::ACCOUNT_TYPE_USER)
    {
     ...
     * @return array result
     */
    protected function _getPrefs($_preferenceName, $_accountId = '0', $_accountType = Rights::ACCOUNT_TYPE_ANYONE)
    {
        ...
        $appId = Application::getInstance()->getApplicationByName($this->_application)->getId();
     ...
     *
     * @param RecordSet $_preferences
     */
    public function getMatchingPreferences(RecordSet $_preferences)
    {
        $_preferences->addIndices(array('name'));

        // get unique names, the matching preference and add it to result set
        $result = new RecordSet('Tinebase_Model_Preference');
    ...
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Deleting pref ' . $_preferenceName);

        $where = array(
            $this->_db->quoteInto($this->_db->quoteIdentifier('name')           . ' = ?', $_preferenceName),
            $this->_db->quoteInto($this->_db->quoteIdentifier('account_id')     . ' = ?', Core::getUser()->getId()),
            $this->_db->quoteInto($this->_db->quoteIdentifier('account_type')   . ' = ?', Rights::ACCOUNT_TYPE_USER)
        );
    ...
    public function create(Tinebase_Record_Interface $_record)
    {
        ...
        if ($_record->personal_only && $_record->account_type == Rights::ACCOUNT_TYPE_ANYONE) {
            $message = 'It is not allowed to set this preference for anyone.';
            Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' ' . $message);
     ...
     * @return Tinebase_Model_Preference
     */
    protected function _getMatchingPreference(RecordSet $_preferences)
    {
        //if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' ' . print_r($_preferences->toArray(), TRUE));
        ...
        if (count($_preferences) == 1) {
            ...
            // check user
            $user = $_preferences->filter('account_type', Rights::ACCOUNT_TYPE_USER);
            if (count($user) > 0) {
                $result = $user->getFirstRecord();
            } else {
                // check group
                $group = $_preferences->filter('account_type', Rights::ACCOUNT_TYPE_GROUP);
                if (count($group) > 0) {
        ...
        if ($result->type !== Tinebase_Model_Preference::TYPE_DEFAULT && is_object(Core::getUser())) {
    ...
    protected function _getDefaultPreference($_preferenceName, $_preferences = NULL)
    {
        if ($_preferences !== NULL) {
            ...
            $defaults = $this->search(new Tinebase_Model_PreferenceFilter(array(array(
                ...
                'operator'  => 'equals',
                'value'     => Application::getInstance()->getApplicationByName($this->_application)->getId()
            ))));
        }
    ...
    protected function _getDefaultContainerOptions($_appName = NULL)
    {
        ...
        $myContainers = Tinebase_Container::getInstance()->getPersonalContainer(Core::getUser(), $appName, Core::getUser(), Tinebase_Model_Grants::GRANT_ADD);
        $sharedContainers = Tinebase_Container::getInstance()->getSharedContainer(Core::getUser(), $appName, [Tinebase_Model_Grants::GRANT_ADD, Tinebase_Model_Grants::GRANT_READ], false, true);
    ...
    protected function _getDefaultContainerPreferenceDefaults(Tinebase_Model_Preference $_preference, $_accountId, $_appName = NULL, $_optionName = self::DEFAULTCONTAINER_OPTIONS)
    {
        ...
        $accountId = ($_accountId) ? $_accountId : Core::getUser()->getId();
