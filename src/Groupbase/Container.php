 ...
 * 
 * any record is tied to a container. the rights of an account on a record gets 
 * calculated by the grants given to this account on the container holding the record (if you know what i mean ;-))
 * 
 * @package     Groupbase
 * @subpackage  Acl
 */
class Container extends AbstractSql implements SearchInterface, ContainerInterface
{
    use ModlogTrait;
    ...
    protected function _addSearchAclFilter(Tinebase_Model_Filter_FilterGroup $_filter)
    {
        ...
        if (! $_filter instanceof Tinebase_Model_ContainerFilter) {
            throw new InvalidArgument('filter is expected to be instanceof Tinebase_Model_ContainerFilter');
        }
        if (count($_filter->getFilter('application_id', true)) !== 1 ||
            ...
            throw new InvalidArgument('filter needs to contain exactly 1 application_id equals filter');
        }
        if ($_filter->getCondition() !== Tinebase_Model_Filter_FilterGroup::CONDITION_AND) {
            throw new InvalidArgument('outermost filter needs to be of condition AND');
        }
        $_filter->addFilter(new Tinebase_Model_Filter_Container('id', 'equals', '/',
            array('applicationName' => Application::getInstance()->
     ...
     * @param   RecordSet $_grants the grants for the new folder 
    ...
    public function addContainer(Tinebase_Model_Container $_container, $_grants = NULL, $_ignoreAcl = FALSE)
    {
        ...
        if ($_ignoreAcl !== TRUE) {
            switch ($_container->type) {
                ...
                case Tinebase_Model_Container::TYPE_SHARED:
                    $application = Application::getInstance()->getApplicationById($_container->application_id);
                    ...
                    if (@class_exists($appAclClassName)) {
                        ...
                        if (in_array(Tinebase_Acl_Rights::MANAGE_SHARED_FOLDERS, $allRights)) {
                            $manageRight = Core::getUser()->hasRight($appName, Tinebase_Acl_Rights::MANAGE_SHARED_FOLDERS);
                        }
                    }
                    

                    if (!$manageRight && !Core::getUser()->hasRight($appName, Tinebase_Acl_Rights::ADMIN)) {
                        throw new Tinebase_Exception_AccessDenied('Permission to add shared container denied.');
                    }
                    break;
                    
                default:
                    throw new InvalidArgument('Can add personal or shared folders only when ignoring ACL.');
                    break;
            }
        }
        
        if (! empty($_container->owner_id)) {
            $accountId = $_container->owner_id instanceof ModelUser ? $_container->owner_id->getId() : $_container->owner_id;
        } else {
            $accountId = (is_object(Core::getUser())) ? Core::getUser()->getId() : NULL;
        ...
        ModificationLog::setRecordMetaData($_container, 'create');
        $container = $this->create($_container);
        $this->setGrants($container->getId(), $event->grants, TRUE, FALSE);

        if (Core::isLogLevel(LogLevel::INFO)) Core::getLogger()->info(__METHOD__ . '::'
        ...
        if(is_string($_recordClass)) {
            ...
            switch(count($split)) {
                case 1:
                    ...
                    if(! $ret['recordClass'] = Core::getApplicationInstance($_recordClass, '', true)->getDefaultModel()) {
                ...
                case 3: 
                    ...
                    break;
                default: throw new InvalidArgument('Invalid value as recordClass given: ' . print_r($_recordClass, 1));
            }
            ...
            throw new InvalidArgument('Invalid value as recordClass given: ' . print_r($_recordClass, 1));
        }
    ...
    protected function _setRecordMetaDataAndUpdate($container, $action)
    {
        ...
        ModificationLog::getInstance()->setRecordMetaData($container, $action, $container);
     ...
     * 
     * @param   string|ModelUser          $accountId
    ...
    public function getContainerByACL($accountId, $recordClass, $grant, $onlyIds = FALSE, $ignoreACL = FALSE)
    {
        ...
        $applicationId = Application::getInstance()->getApplicationByName($meta['appName'])->getId();
        ...
        if ($onlyIds) {
            ...
            $result = new RecordSet('Tinebase_Model_Container', $stmt->fetchAll(Zend_Db::FETCH_ASSOC), true);
        }
        ...
        if (empty($result)) {
            ...
            if ($personalContainer instanceof Tinebase_Model_Container) {
                $result = ($onlyIds) ? 
                    ...
                    new RecordSet('Tinebase_Model_Container', array($personalContainer), TRUE);
            }
        }
    ...
    public function getContainerByName($recordClass, $containerName, $type, $ownerId = NULL)
    {
        ...
        $ownerId = $ownerId instanceof ModelUser ? $ownerId->getId() : $ownerId;
    ...
    public function getPersonalContainer($_accountId, $_recordClass, $_owner, $_grant = Tinebase_Model_Grants::GRANT_READ, $_ignoreACL = false)
    {
        ...
        $accountId   = ModelUser::convertUserIdToInt($_accountId);
        $ownerId     = ModelUser::convertUserIdToInt($_owner);
        $grant       = $_ignoreACL ? '*' : $_grant;
        $application = Application::getInstance()->getApplicationByName($meta['appName']);
        ...
        if (empty($containersData) && $accountId === $ownerId) {
            $application = Core::getApplicationInstance($meta['appName'], '', true);

            if ($application instanceof Application_Container_Interface && method_exists($application, 'createPersonalFolder')) {
        ...
        $containers = new RecordSet('Tinebase_Model_Container', $containersData, TRUE);
     ...
     * @param ModelUser|string $account
    ...
    {
        if (! $account instanceof ModelUser) {
        ...
        $container = $this->addContainer(new Tinebase_Model_Container(array(
            ...
            'backend'           => 'Sql',
            'model'             => $recordClass,
            'application_id'    => Application::getInstance()->getApplicationByName($applicationName)->getId()
        )));
     ...
     * @param   string|ModelUser          $accountId use current user if omitted
    ...
    public function getDefaultContainer($recordClass, $accountId = NULL, $defaultContainerPreferenceName = NULL)
    {
        ...
        if ($defaultContainerPreferenceName !== NULL) {
            $defaultContainerId = Core::getPreference($meta['appName'])->getValueForUser($defaultContainerPreferenceName, $account->getId());
            try {
                ...
                Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' Default container not found (' . $te->getMessage() . ')');
                // default may be gone -> remove default adb pref
                $appPref = Core::getPreference($meta['appName']);
                if ($appPref) {
        ...
        if ($defaultContainerPreferenceName !== NULL) {
            // save as new pref
            Core::getPreference($meta['appName'])->setValue($defaultContainerPreferenceName, $result->getId());
        }
    ...
    public function getSharedContainer($_accountId, $recordClass, $_grant, $_ignoreACL = FALSE, $_andGrants = FALSE)
    {
        ...
        foreach (Application::getInstance()->getAllApplicationGrantModels($application) as $grantModel) {
        ...
        $containers = new RecordSet('Tinebase_Model_Container', $data, TRUE);
     ...
     * @param   bool                                $_ignoreACL
     * @param   bool                                $_andGrants
     * @return  RecordSet set of ModelUser
     */
    ...
    protected function _getOtherUsersContainerData($_accountId, $_application, $_grant, $_ignoreACL = FALSE)
    {
        ...
        $containers = new RecordSet('Tinebase_Model_Container', $result, TRUE);
    ...
    public function deleteContainer($_containerId, $_ignoreAcl = false)
    {
        ...
        if ($_ignoreAcl !== TRUE) {
            if(!$this->hasGrant(Core::getUser(), $containerId, Tinebase_Model_Grants::GRANT_ADMIN) && (
                    !isset($container->xprops()['Calendar']['Resource']['resource_id']) ||
                    !Core::getUser()->hasRight('Calendar', Calendar_Acl_Rights::MANAGE_RESOURCES))) {
            ...
            if($container->type !== Tinebase_Model_Container::TYPE_PERSONAL and $container->type !== Tinebase_Model_Container::TYPE_SHARED) {
                throw new InvalidArgument('Can delete personal or shared containers only.');
            }

            // get personal container
            $personalContainer = $this->getDefaultContainer($container->model, Core::getUser());
        ...
        $myTransactionId = $tm->startTransaction(Core::getDb());
        ...
        $app = Application::getApplicationById($container->application_id);
    ...
    public function deleteContainerContents($container, $_ignoreAcl = FALSE)
    {
        ...
        $controller = Core::getApplicationInstance($model, /* $_modelName */ '', $_ignoreAcl);
    ...
    public function setContainerName($_containerId, $_containerName)
    {
        ...
        if (!$this->hasGrant(Core::getUser(), $container, Tinebase_Model_Grants::GRANT_ADMIN)) {
    ...
    public function hasGrant($_accountId, $_containerId, $_grant)
    {
        ...
        try {
            ...
            if (Core::isLogLevel(LogLevel::NOTICE)) Core::getLogger()->notice(__METHOD__ . '::' . __LINE__ . ' ' . $teia->getMessage());
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' ' . $teia->getTraceAsString());
            return false;
        }
     ...
     * @param   int|Tinebase_Model_Container $_containerId
     * @param   bool                         $_ignoreAcl
     * @return  RecordSet subtype Tinebase_Model_Grants
     * @throws  Tinebase_Exception_AccessDenied
     */
    public function getGrantsOfContainer($_containerId, $_ignoreAcl = FALSE)
    {
        ...
        $grants = new RecordSet($grantModel);
        ...
        if ($_ignoreAcl !== TRUE) {
            if (TRUE !== $this->hasGrant(Core::getUser()->getId(), $containerId, Tinebase_Model_Grants::GRANT_ADMIN)) {
     ...
     *
     * @param   RecordSet   $_records records to get the grants for
     * @param   string|ModelUser  $_accountId the account to get the grants for
     * @param   string                      $_containerProperty container property
     * @param   string                      $_grantModel
     * @throws  Tinebase_Exception_NotFound
     */
    public function getGrantsOfRecords(RecordSet $_records, $_accountId, $_containerProperty = 'container_id')
    {
     ...
     */
    public function getContainerGrantsOfRecords(RecordSet $_records, $_accountId, $_containerProperty = 'container_id')
    {
        $containerIds = array();
        foreach ($_records as $record) {
     ...
     */
    public function setGrants($_containerId, RecordSet $_grants, $_ignoreAcl = FALSE, $_failSafe = TRUE) 
    {
        $containerId = Tinebase_Model_Container::convertContainerId($_containerId);
        ...
        if (Core::isLogLevel(LogLevel::INFO)) Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Setting grants for container id ' . $containerId . ' ...');
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' ' . print_r($_grants->toArray(), TRUE));
    ...
    public function checkContainerOwner(Tinebase_Model_Container $_container)
    {
        ...
        if (! $_container->account_grants instanceof RecordSet) {
        ...
        if (count($adminGrants) > 1) {
            Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Multiple admin grants detected in container "' . $_container->name . '"');
            if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' ' . print_r($adminGrants->toArray(), TRUE));
     ...
     * @param RecordSet $grants use this to overwrite default grants
     * @param string $model the model the container contains
     * @return Tinebase_Model_Container
     */
    public function createSystemContainer($application, $name, $configId = NULL, RecordSet $grants = NULL, $model = NULL)
    {
        $application = ($application instanceof Tinebase_Model_Application) ? $application : Application::getInstance()->getApplicationById($application);
        if ($model === null) {
            $controller = Core::getApplicationInstance($application->name, /* $_modelName = */ '', /* $_ignoreACL = */ true);
        ...
        switch ($_modification->change_type) {
            ...
            case ModificationLog::UPDATED:
                ...
                if (isset($diff->diff['account_grants'])) {
                    ...
                    $this->setGrants($container, new RecordSet($container->getGrantClass(), $diff->diff['account_grants']['added']), true, false);
                } else {
        ...
        try {
            ...
            foreach ($this->search($filter) as $container) {
                ...
                if (!isset($modelBackendCache[$container->model])) {
                    $recordsBackend = Core::getApplicationInstance($container->model)->getBackend();
                ...
                if (method_exists($recordsBackend, 'increaseSeqsForContainerId')) {
                    ...
                    if (Core::isLogLevel(LogLevel::INFO)) {
                        Core::getLogger()->info(__METHOD__ . '::' .
