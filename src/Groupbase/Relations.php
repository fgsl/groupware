...
class Relations
{
    ...
    public function undeleteRelations($_relationData)
    {
        foreach((array) $_relationData as $relationData) {
            if ($relationData instanceof Relation) {
                ...
                $relation = new Relation($relationData, true);
            }

            $relation->related_record = null;

            try {
                $appController = Core::getApplicationInstance($relation->related_model);
                try {
            ...
            ModificationLog::setRecordMetaData($relation, 'undelete', $relation);
     ...
     * @param  array|RecordSet  $_relationData    data for relations to create
    ...
    {
        if ($_relationData instanceof RecordSet) {
            $relations = $_relationData;
        } else {
            $relations = new RecordSet('Relation');
            foreach ((array)$_relationData as $relationData) {
                if ($relationData instanceof Relation) {
                    $relations->addRecord($relationData);
                } else {
                    $relation = new Relation(NULL, TRUE);
            ...
            if ($_inspectRelated && isset($current->related_record) && isset($update->related_record)) {
                ...
                if (! $current->related_record->isEqual(
                    ...
                    if ( !$update->related_record->has('container_id') ||
                        Tinebase_Container::getInstance()->hasGrant(Core::getUser()->getId(), $update->related_record->container_id,
     ...
     *
     * @param RecordSet $relations
     * @param RecordSet $currentRelations
     * @return mixed
     */
    ...
    public static function getConstraintsConfigs($models)
    {
        ...
        $allApplications = Application::getInstance()->getApplicationsByState(Application::ENABLED)->name;
        $ret = array();
            ...
            foreach($cc['config'] as $config) {
                ...
                if (isset($myConstraints[$idGroup]) && ($config['max'] > 0 && $config['max'] < $myConstraints[$idGroup])) {
                    ...
                    if (Core::isLogLevel(LogLevel::INFO)) {
                        Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Constraints validation failed from the own side! ' . print_r($cc, 1));
                    }
                ...
                foreach($relatedIds as $relatedId) {
                    ...
                    if (isset($allConstraints[$idGroup]) && ($config['max'] > 0 && $config['max'] < $allConstraints[$idGroup])) {
                        ...
                        if (Core::isLogLevel(LogLevel::INFO)) {
                            Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' Constraints validation failed from the other side! ' . print_r($cc, 1));
                        }
     ...
     * @return RecordSet of Relation
     */
    public function getRelations($_model, $_backend, $_id, $_degree = NULL, array $_type = array(), $_ignoreACL = FALSE, $_relatedModels = NULL)
    {
        if (Core::isLogLevel(LogLevel::TRACE)) Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . "  model: '$_model' backend: '$_backend' " 
     ...
     * @return array  key from $_ids => RecordSet of Relation
     */
    public function getMultipleRelations($_model, $_backend, $_ids, $_degree = NULL, array $_type = array(), $_ignoreACL = FALSE, $_relatedModels = NULL)
    {
        ...
        foreach ($flippedIds as $key) {
            $result[$key] = new RecordSet('Relation', array(),  true);
        }
     ...
     *
     * @param  Relation|RecordSet
     * @throws InvalidArgument
     */
    protected function _relatedRecordToObject($_relations)
    {
        if(! $_relations instanceof RecordSet) {
            $_relations = new RecordSet('Relation', array($_relations));
        }
        
        foreach ($_relations as $relation) {
            if (! is_string($relation->related_model)) {
                throw new InvalidArgument('missing relation model');
            }
     ...
     * 
     * @param   RecordSet $_relation of Relation
    ...
    protected function _setAppRecord($_relation, $_doCreateUpdateCheck = false)
    {
        ...
        $appController = Core::getApplicationInstance($_relation->related_model);
        ...
        if ($method === 'update' && $appController->doContainerACLChecks()
            && ! Core::getUser()->hasGrant($_relation->related_record->container_id, Tinebase_Model_Grants::GRANT_EDIT)
        ) {
            if (Core::isLogLevel(LogLevel::NOTICE)) Core::getLogger()->notice(__METHOD__ . '::' . __LINE__
        ...
        switch ($_relation->related_model) {
            ...
            case 'Tasks_Model_Task':
                ...
                break;
            default:
                $_relation->related_backend = Relation::DEFAULT_RECORD_BACKEND;
                break;
        }
    }
     ...
     * 
     * @param  RecordSet $_relations of Relation
     * @param  boolean $_ignoreACL 
     * @return void
     * 
     * @todo    make getApplicationInstance work for tinebase record (ModelUser for example)
     */
    protected function resolveAppRecords($_relations, $_ignoreACL = FALSE)
    {
        ...
        foreach ($_relations as $relation) {
            if (!(isset($modelMap[$relation->related_model]) || array_key_exists($relation->related_model, $modelMap))) {
                $modelMap[$relation->related_model] = new RecordSet('Relation');
            }
        ...
        foreach ($modelMap as $modelName => $relations) {
            ...
            if (class_exists($rightClass)) {
                ...
                if (is_object($u) && $ref->hasConstant(strtoupper($rightName)) && (! $u->hasRight($split[0], $rightName)) && (! $u->hasRight($split[0], Tinebase_Acl_Rights::ADMIN))) {
                    if (Core::isLogLevel(LogLevel::DEBUG)) {
                        ...
                        Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Skipping relation due to no manage right: ' . $modelName);
                    }
                    continue;
                }
            }
            ...
            $records = null;
            $removeReason = Relation::REMOVED_BY_OTHER;
            if ($modelName === 'ModelUser') {
                ...
                try {
                    $appController = Core::getApplicationInstance($modelName);
                    if (method_exists($appController, $getMultipleMethod)) {
                        ...
                        throw new AccessDenied('Controller ' . get_class($appController) . ' has no method ' . $getMultipleMethod);
                    }
                } catch (AccessDenied $tea) {
                    ...
                    $removeReason = Relation::REMOVED_BY_ACL;
                } catch (NotFound $tenf) {
                    Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' could not find controller for model: ' . $modelName . '! you have broken relations: ' . join(',', $relations->id));
                    $_relations->removeRecords($relations);
                    continue;
                } catch (Tinebase_Exception_AreaLocked $teal) {
                    $removeReason = Relation::REMOVED_BY_AREA_LOCK;
                    if (Core::isLogLevel(LogLevel::INFO)) Core::getLogger()->info(__METHOD__ . '::' . __LINE__
            ...
            foreach ($relations as $relation) {
                $recordIndex    = $records instanceof RecordSet
     ...
     * adds a new relation
     * 
     * @param   Relation $_relation
     * @return  Relation|NULL the new relation
     * @throws  Tinebase_Exception_Record_Validation
     */
    protected function _addRelation(Relation $_relation)
    {
        $_relation->created_by = Core::getUser()->getId();
        ...
        try {
            ...
            Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Could not add relation: ' . $zse->getMessage());
            $result = NULL;
        }
     ...
     * 
     * @param  Relation $_relation 
     * @return Relation the updated relation
     */
    protected function _updateRelation($_relation)
    {
        $_relation->last_modified_by = Core::getUser()->getId();
    ...
    {
        if (! Core::getUser()->hasRight('Tinebase', Tinebase_Acl_Rights::ADMIN)) {
            throw new AccessDenied('Only Admins are allowed to perform his operation!');
        }
    ...
    public function getRelationsOfRecordByDegree(Tinebase_Record_Interface $record, $degree, $ignoreACL = FALSE)
    {
        ...
        $result = new RecordSet('Groupbase\Model\Relation');
