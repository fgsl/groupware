...
use Fgsl\Groupware\Groupbase\ModelConfiguration\NewModelConfiguration as MC;
use Zend\InputFilter\Input;
use Fgsl\Groupware\Groupbase\Model\Container as ModelContainer;
use Fgsl\Groupware\Groupbase\Record\NewAbstract;
...
use Fgsl\Groupware\Groupbase\Model\Filter\ForeignId;
...
use Fgsl\Groupware\Addressbook\Controller\ListRole as ControllerListRole;
use Fgsl\Groupware\Groupbase\Model\PathFilter;
use Fgsl\Groupware\Groupbase\Model\Path as ModelPath;
use Fgsl\Groupware\Groupbase\Record\Path as RecordPath;
 ...
 */
class Contact extends NewAbstract
{
    /**
    ...
    protected static $_modelConfiguration = [
        ...
        'associations' => [
            \Doctrine\ORM\Mapping\ClassMetadataInfo::MANY_TO_ONE => [
                'container_id_fk' => [
                    'targetEntity' => ModelContainer::class,
        ...
        self::FIELDS        => [
            ...
            'adr_one_countryname'           => [
                ...
                self::INPUT_FILTERS             => [StringTrim::class, StringToUpper::class],
            ],
            'adr_one_locality'              => [
                self::TYPE                      => self::TYPE_STRING,
                self::LENGTH                    => 64,
            ...
            'adr_one_lon'                   => [
                ...
                self::VALIDATORS                => [Input::ALLOW_EMPTY => true],
                self::INPUT_FILTERS             => [NotEmpty::class => true],
            ],
            'adr_one_lat'                   => [
                self::TYPE                      => self::TYPE_FLOAT,
                self::NULLABLE                  => true,
            ...
            'email'                         => [
                ...
                self::VALIDATORS                => [Input::ALLOW_EMPTY => true],
                self::INPUT_FILTERS             => [StringTrim::class, StringToLower::class],
                self::QUERY_FILTER              => true,
            ],
            'email_home'                    => [
                self::TYPE                      => self::TYPE_STRING,
            ...
            'industry'                      => [
                ...
                self::FILTER_DEFINITION         => [
                    self::FILTER                    => ForeignId::class,
                    self::OPTIONS                   => [
                        self::FILTER_GROUP              => IndustryFilter::class,
                        self::CONTROLLER                => Industry::class
                    ]
                ]
            ],
            'jpegphoto'                     => [
            ...
            'paths'                         => [
                ...
                'config'                        => [
                    ...
                    'filterClassName'               => PathFilter::class,
                ],
                'label'                         => 'Paths', // _('Paths')
            ...
            'salutation'                    => [
                ...
                self::VALIDATORS                => [Input::ALLOW_EMPTY => true],
                self::NAME                      => Config::CONTACT_SALUTATION,
            ],
            'syncBackendIds'                => [
                self::TYPE                      => self::TYPE_TEXT,
                self::LENGTH                    => 16000,
            ...
            'type'                          => [
                ...
                self::VALIDATORS                => [
                    Input::ALLOW_EMPTY      => true,
                    Input::DEFAULT_VALUE    => self::CONTACTTYPE_CONTACT,
            ...
            'url'                           => [
                ...
                self::VALIDATORS                => [Input::ALLOW_EMPTY => true],
                self::INPUT_FILTERS             => [StringTrim::class],
            ],
            'url_home'                      => [
                self::TYPE                      => self::TYPE_STRING,
                self::LENGTH                    => 128,
    ...
    public static function normalizeTelephoneNum($telNumber)
    {
        ...
        try {
            $numberFormat = $phoneUtil->parse($telNumber, Config::getInstance()
                ->{Config::DEFAULT_TEL_COUNTRY_CODE});
    ...
    protected function _setContactImage(&$_data)
    {
        ...
        if (Core::isLogLevel(LogLevel
            ::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' image params:' . print_r($imageParams, true));
        if ($imageParams['isNewImage']) {
            try {
                $_data['jpegphoto'] = Tinebase_ImageHelper::getImageData($imageParams);
            } catch(UnexpectedValue $teuv) {
                Core::getLogger()->warn(__METHOD__ . '::' . __LINE__ . ' Could not add contact image: ' . $teuv->getMessage());
    ...
    public function setSmallContactImage($newPhotoBlob, $maxSize = self::SMALL_PHOTO_SIZE)
    {
        if ($this->getId()) {
            try {
                $currentPhoto = Controller::getInstance()->getImage('Addressbook', $this->getId())->getBlob('image/jpeg', $maxSize);
            } catch (Exception $e) {
                // no current photo
            }
        }
    ...
    public function getPathNeighbours()
    {
        ...
        if (count($listRoles) > 0) {
            $listRoles = ControllerListRole::getInstance()->getMultiple($listRoles, true)->asArray();
        }
