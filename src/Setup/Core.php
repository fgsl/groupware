<?php
namespace Fgsl\Groupware\Setup;

use Fgsl\Groupware\Groupbase\Core as GroupbaseCore;
 ...
 * @package     Setup
 */
class Core extends GroupbaseCore
{
    /**
    ...
    public static function initFramework()
    {
        ...
        Core::set('locale', 'en_US');
        Core::set(GroupbaseCore::USERTIMEZONE, 'UTC');
     ...
     * startSetupSession
     * 
     * TODO remove redundancy with Core::startCoreSession()
     */
     ...
     * @return boolean
     * 
     * @todo try to write to db, if it fails: self::set(Core::CHECKDB, FALSE);
     */
    public static function setupDatabaseConnection()
    {
        ...
        if (self::configFileExists()) {
            $dbConfig = Core::getConfig()->database;
            ...
            try {
                ...
                Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' ' . $zae->getMessage());
            } catch (Zend_Db_Exception $zde) {
                Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . ' ' . $zde->getMessage());
            }
        }
        
        self::set(Core::CHECKDB, $dbcheck);
        return $dbcheck;
    }
