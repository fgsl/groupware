...
abstract class AbstractBackend implements BackendInterface
{
    ...
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Truncate table ' . $_tableName);
        ...
        foreach ($_record->field as $field) {
            if (isset($field->value['special'])) {
                switch(strtolower($field->value['special'])) {
                    ...
                    case 'application_id':
                        $application = Application::getInstance()->getApplicationByName((string) $field->value);
    ...
    public function createTable(Setup_Backend_Schema_Table_Abstract $_table, $_appName = NULL, $_tableName = NULL)
    {
        ...
        if ($_appName !== NULL && $_tableName !== NULL) {
            Application::getInstance()->addApplicationTable(
                Application::getInstance()->getApplicationByName($_appName), 
                $_tableName, 
                1
            );
        }
    ...
    {
        if (Core::isLogLevel(LogLevel::DEBUG)) Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Dropping table ' . $_tableName);
        $statement = "DROP TABLE IF EXISTS " . $this->_db->quoteIdentifier(SQL_TABLE_PREFIX . $_tableName);
        $this->execQueryVoid($statement);
        
        if ($_applicationId !== NULL) {
            Application::getInstance()->removeApplicationTable($_applicationId, $_tableName);
        }
    }
    ...
    public function dropIndex($_tableName, $_indexName)
    {
        ...
        try {
            ...
            if (\Fgsl\Groupware\Setup\Core::isLogLevel(LogLevel::NOTICE)) Core::getLogger()->notice(__METHOD__ . '::' . __LINE__
                . ' ' . $zdse);
    ...
    {
        if (strlen($_name) > AbstractBackend::MAX_NAME_LENGTH) {
            $_name = substr(md5($_name), 0 , AbstractBackend::MAX_NAME_LENGTH);
        }
        return $_name;
    }
}
